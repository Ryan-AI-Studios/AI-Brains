...
- **Research date:** 2026-08-29 (plan-write product HEAD `ed2f5f8` T316 Completed note `#240`; T316 product `#239` `4d1a53e`). Snapshot — **re-verify at execute**.
- **Ledger:** planning DOCS TX `156b2a03-b5aa-4905-b840-d14fb182aa90`. Series mint DOCS `a6d3c404-1d64-4cba-a743-d75ac16c74cd`. Implement starts a **FEATURE** TX on **go**.
...
| Signal | Observation |
|--------|-------------|
| HEAD | `ed2f5f8` `chore(conductor): T316 Completed note with PR #239 squash 4d1a53e (#240)`. Tree **CLEAN** at plan start. Branch `track/T318-backup-list-usable-first` off `main`. `origin/main` = `ed2f5f8` (ahead **0**). |
...
| **F9 — T225 helpers** | `VERIFY_FAIL_PREVIEW_CAP=5`, `format_verify_counts`, `should_emit_create_nudge`, `format_create_nudge` freeze. Mixed-success adds a small trailer helper or inline format in `run_verify` / `verify_report.rs`. |
...
| **F24 — Stay-green** | `list_sort_key` units; T198 empty list/verify; T225 zero-OK 5-FAIL + nudge; T225 verbose mixed stream; T225 JSON full results; T295 create after_help; T277 mixed doctor `backup_recent` Ok; verify exit 1 any FAIL; quiet-wins. |
...
| **AC3** | All-residual Default (plain only): stdout contains `No usable backups.` + footer count; stdout table has no `(legacy plain)` row | New hermetic `backup_list__all_residual__no_usable_and_footer` (updates T209 AC1 token-on-Default) |
...
| **AC5** | `--quiet` mixed: usable row present; no footer SOOT on stdout **or** stderr; dual `--quiet --verbose` quiet wins | Update `backup_list_honesty__quiet__no_summary` |
| **AC6** | Empty backups dir: `No backups found.` exit 0 | Stay-green T198 |
| **AC7** | `list_sort_key` units unchanged | Stay-green `list_sort_tests` |
| **AC8** | Verify mixed 1 OK + 1 FAIL: counts contain `1 OK` and `1 FAIL`; **no** `FAIL —`; trailer contains `--verbose`; exit **1**; **no** create nudge | Flip `backup_verify_all__mixed__reports_per_file` + stay-green `backup_verify__mixed_ok_and_key_mismatch__one_ok_exit_1_no_nudge` |
...
- else if `fail >= 1`: `println!("{fail} FAIL (use --verbose for per-file).");` (no preview lines, no T225 overflow trailer — this line *is* the trailer).
...
- Smoke mixed verify `FAIL —` **absent** (AC8) — today requires present.

**Green:** filter Default/Quiet rows; `println!` F6; gate verify preview on `ok==0`; List after_help.

**Stay-green:** AC4/AC6/AC7/AC9–AC13/AC16/AC18/AC19-doctor.
...
| T209 AC1/AC2/AC3 bitrot | Same-commit hermetic updates (AC1–AC5) |
...
1. Phase 0 re-read `run_list` / F6 / `run_verify` default arm / `verify_report.rs` / T209+T244 hermetics / T277 mixed list / smoke mixed verify; rescan deferred; FEATURE TX; record live N.
2. Red hermetics AC1–AC5/AC8/AC14 (must fail).
3. Green: usable-only emit + stdout footer + verify mixed gate + List after_help.
4. Stay-green AC6/AC7/AC9–AC13/AC16/AC18; flip T209/T244/T277 stderr+row asserts.
...
| Path | Change |
|------|--------|
| `crates/ai-brains-cli/src/commands/backup.rs` | Default/Quiet usable-only rows; F6 `println!`; `run_verify` mixed gate; maybe tiny footer helper |
| `crates/ai-brains-cli/src/verify_report.rs` | Optional mixed-success trailer helper + unit |
| `crates/ai-brains-cli/src/main.rs` | `BackupCommands::List` after_help one sentence only |
| `crates/ai-brains-cli/tests/backup_list_honesty.rs` | AC1–AC5 row/stream updates; AC14 if help lives here or new test |
...
## 13. Planning snapshot

Plan-write HEAD `ed2f5f8`. Ahead of `origin/main`: **0** at branch cut (docs commit will be ahead **1**). last-PR `#240` empty / `#239` empty / `#237` → T326 / `#230` → T325 / no T327.
