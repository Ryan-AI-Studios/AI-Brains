...
**Status:** Series active — **T217 + T218 + T220 + T221 closed**; **T219 Planning** (plan-only until go); remaining tracks placeholders.
**Prior series:** T205–T216 closed (skill·CLI honesty).
**Closed:** **T217** FTS multi-token rescue (PR #110 `1e22e77`); **T218** semantic quality v2 (PR #116 `fc4d370`); **T220** preflight summary JSON honesty (PR #112 `6f4f67b`); **T221** governed first-run + deny exit honesty (PR #114 `b3c4b0f`, 2026-08-09).
**Next honesty:** **T219** preflight pretty (**Planning** — newline-preserving `trim_to_word_budget` + Scope + role strip + section caps) / **T224** peers.
...
| Preflight pretty wall (8 / **5**) | **T219** Preflight pretty readability (**Planning**) | P2 |
...
2. ~~**T217**~~ **closed**; ~~**T218**~~ **closed** PR #116; ~~**T220**~~ **closed**; ~~**T221**~~ **closed**.
3. **T219** (pretty — **Planning**; implement on go), then **T224** (role strip; reuses T219 strip helper).
