...
**Status:** **Pending** (Planned). Implement only on **go**.
...
- [ ] Re-dogfood `graph update --format human` + `doctor --summary` + `daemon status` + `graph rebuild --help` read-only — **do not rebuild live** until owner confirms
- [ ] Pins clap **4.6.1** / rusqlite **0.39.0** — no bump; no new crate
- [ ] FEATURE TX
- [ ] Did **not** `cargo install`; did **not** grow `doctor.rs` / `graph_density.rs` / `rebuild.rs` / hotspots; did **not** stop daemon without owner
- [ ] Rescan `conductor/deferred.md` — T300 absorbed; floors / clap 5 / leftover `--write` not stolen

---

## Absorbed deferred

- [ ] Audit graph sparse E/N ~0.14 live rebuild → F1–F8 / AC1–AC5 / AC14
- [ ] Placeholder Manual `graph update` + owner-confirm `graph rebuild` + doctor agree → AC14 / F3
- [ ] Placeholder floors frozen; never force `live` → F2 / F3
- [ ] Placeholder skip = T262 hermetic + written skip → F1 / F11 / AC6
- [ ] T278 F8 Stop-Before → lift to owner-confirm F1 (T295 class)
...
- [ ] `graph_rebuild__dry_run__prints_density_no_mutation` (AC1)
- [ ] `graph_rebuild__mutate__prints_density_and_keeps_pin_node` (AC2)
- [ ] `rebuild_with_daemon_state__daemon_up_mutate__err` (AC3)
- [ ] `graph_rebuild__format_json__health_keys` (AC5)

---

## Phase 2 — green

- [ ] `pub(crate) fn rebuild_daemon_busy_message()` T188 substrings
- [ ] `rebuild_with_daemon_state(ctx, dry_run, format, daemon_up)`
- [ ] `async fn rebuild` → `probe_restore_daemon_busy` then core
- [ ] Shared `graph_health_report` used by `update` + rebuild
- [ ] Dry-run: health + human `[dry-run]` + COUNT `SELECT COUNT(*) FROM events` fail-open; **no** `GraphRebuilder`
- [ ] Mutate: `GraphRebuilder::rebuild()` then health emit
- [ ] clap `Rebuild { dry_run, format }` default human; tokens `human\|json`; dispatch `.await`
- [ ] `rebuild.rs` / `graph_density.rs` / `doctor.rs` production **unchanged**
- [ ] Stay-green AC6 T262 / AC7 feature-off / AC8 floors+15 / AC9 clap / AC15 update / AC16 peers

---

## Phase 3 — docs

- [ ] CAPABILITIES rebuild row additive (`:461` at plan)
- [ ] OPERATIONS Graph health extend (daemon stop; `--dry-run`; stdout density; may stay sparse)
- [ ] WORKFLOWS `:215` daemon-stop before rebuild
- [ ] Root CHANGELOG T300 Unreleased
- [ ] Rebuild variant `after_help` (AC11)
- [ ] CLI-EXIT-CODES **add** daemon-up exit 1; still-sparse success exit 0
- [ ] PROTOCOL-COMPAT `:96` rebuild JSON = update keys

---

## Phase 4 — verify

- [ ] Targeted nextest `-p ai-brains-cli` `graph_rebuild` / `graph_live_projection` / `exit_contract` graph + `-p ai-brains-graph` rebuild idempotent
- [ ] `cargo clippy -p ai-brains-cli --all-targets --features graph -- -D warnings`
- [ ] Manual AC14 `--dry-run` via **`cargo run -p ai-brains-cli --features graph`**. Mutate **only** if owner confirmed daemon stop. Pass-with-observed-data.
...
- [ ] conductor T300 **Completed**
- [ ] deferred.md T300 closeout table (unresolved lows / live skip)
- [ ] README-T285-T300 T300 Completed (series closer)
...
- [ ] Mutating rebuild fail-closes when daemon up (T188 substrings); `--dry-run` allowed with NOTICE
- [ ] Successful rebuild prints density (human default; JSON = update keys); exit 0 even if still `sparse`
- [ ] T213 floors unchanged; doctor still 15 checks; T232 remediator string unchanged; no `--confirm`
- [ ] T262 pin→neighbors without rebuild stay-green
- [ ] Manual AC14 recorded (`--dry-run` always; live mutate only with owner confirm)
- [ ] Full gate green; contracts/pins unchanged; `rebuild.rs` / `graph_density.rs` / `doctor.rs` production unchanged
- [ ] Published (Phase 6)
