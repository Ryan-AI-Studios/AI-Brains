...
- [ ] FEATURE TX
- [ ] Did **not** `cargo install`; did **not** grow `project.rs` / CLI `preflight.rs` / `doctor.rs` / `sync.rs` / `policy_cmd.rs`
...
- [ ] CLI-EXIT-CODES POLICY_DENIED paragraph (F19)
- [ ] CAPABILITIES progressive/bootstrap omit note
- [ ] CHANGELOG T280
- [ ] PROTOCOL-COMPAT: no new required keys
- [ ] Skill one-liner if policy bootstrap section exists
- [ ] conductor Completed on implement closeout

## Phase 4 — Verify

- [ ] Targeted nextest: policy_bootstrap; CLI governed_common unit; ai-brainsd AC11; CP query/renderer; T275 renderer
...
- [ ] Classify-only live `cargo run -p ai-brains-cli -- policy check --capability ReadEvidence` (AC10). **No** live bootstrap

## DoD (checkable)

- [ ] Three HINT copies byte-equal F1 (AC1–AC3)
- [ ] Markdown next = SHORT; no `--scope …` (AC4)
- [ ] Hermetic deny hint omits `--scope …` (AC5)
- [ ] T210 AC8 no-context fail_usage still names `--scope` (AC6)
- [ ] Doctor LONG / show SHORT unchanged (AC7)
- [ ] T275 grant-wall green (AC8)
- [ ] Progressive deny no `--scope …` (AC9)
- [ ] Live classify-only AC10 (`cargo run`, not PATH)
- [ ] No live `policy bootstrap` unless owner confirmed
- [ ] No `cargo install`
- [ ] Diff omits `project.rs` / CLI `preflight.rs` / `doctor.rs` / `sync.rs` / `policy_cmd.rs` (AC12)
- [ ] implement-track Phase 6: push `track/T280-*` → PR → watch GHA `CI` green → squash-merge → prune (never `git push origin main`)
