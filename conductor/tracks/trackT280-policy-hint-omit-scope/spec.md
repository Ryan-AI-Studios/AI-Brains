...
- **Research date:** 2026-08-22 (plan dogfood HEAD `83080ff` T279 `#195`; product `src/` = T279).
- **Ledger:** planning DOCS TX `e51b3b28-d885-46cd-b622-3a7b82ae489a`. Implement starts a **FEATURE** TX on **go**.
...
| CP progressive twin | `control-plane/src/query.rs` **`:93`** (815 lines) | Local const, T221 F17. Same literal. Add unit locking F1 (no CLI dep). |
...
| T210 AC8 | `tests/policy_bootstrap.rs` **`:546`** | `--no-project-context` omit `--scope` → fail_usage exit 2. **Stay green** — this is the no-context arm. |
...
| ID | Decision |
|----|----------|
| **F0 — Go gate** | Plan-only until user **go**. Planning is DOCS TX `e51b3b28`. Implement starts a **FEATURE** TX. |
...
| **F22 — PowerShell** | `;` not `&&`. |
| **F23 — Existing tests stay green** | T210 AC1–AC8 (AC7 tightened, not dropped); T221 progressive deny + after-bootstrap; T226 soft-resolve; T241 doctor LONG / show SHORT / check catalog; T243 recall fallback; T263 Personal deny; T275 grant-wall + AC16; `render_project_markdown__denied__bootstrap_next_step_no_empty_authority` (next still precedes Decisions; string is SHORT). |
...
| AC | Proof |
|----|-------|
| **AC1** | Unit: CLI `POLICY_DENIED_HINT` equals F1 literal; does **not** contain `--scope …`; contains `omit --scope` and `policy bootstrap` and `--dry-run`. **Required red** (`policy_denied_hint__wording__omits_required_scope`). |
| **AC2** | Unit: daemon `POLICY_DENIED_HINT` byte-equal to F1; `policy_denied_with_hint` details.hint matches. **Required red.** |
| **AC3** | Unit: CP `query.rs` const byte-equal to F1 (new or existing `#[cfg(test)]`). **Required red.** |
| **AC4** | Unit: `BRIEFING_DENIED_NEXT_STEP == BRIEFING_DENIED_DENIAL_HINT`; neither contains `--scope …`; markdown denied still contains `policy bootstrap` and T275 grant-wall; next precedes `## Decisions`. **Required red.** |
...
| HINT length (~183 chars) | No historical 140 cap on HINT (T275 140 is grant-wall). Substring tests, not char budget. |
...
1. Phase 0 re-verify HINT `:51/:989/:93`, NEXT_STEP `:13`, SHORT/LONG, T210 AC7/AC8, T243 unit, T275 grant-wall, deferred rescan, #195 still empty, pins.
2. Red AC1–AC5.
3. Three HINT copies (F1) + T243 rename (F27) + daemon AC11 tighten (F28).
4. Renderer NEXT_STEP = DENIAL_HINT (F2).
...
| `crates/ai-brains-control-plane/src/query.rs` | F1 twin + AC3 unit |
