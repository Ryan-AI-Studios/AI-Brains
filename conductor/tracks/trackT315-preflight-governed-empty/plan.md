...
| Check | Result |
|-------|--------|
| HEAD / tree | `44520d8` T312 `#230` CLEAN. Branch `track/T315-preflight-governed-empty`. `origin/main` = HEAD. |
...
| T241 | optional `next_step` only when grants incomplete (`:908–912`) |
| rustc | **1.95.0** |
...
- [ ] Confirm `LIST_RECALL_QUERY` still `"what did we decide"` in `governed_common.rs`
...
- [ ] Insert helper after word-count / before footer (AC4)
- [ ] Trigger unit hotspots=5 / decisions=0 still inserts (AC14)
- [ ] CLI hermetic `preflight_summary_json.rs` empty-vault human + JSON (AC5/AC6)
- [ ] Confirm those tests **fail** on current tree (`Total Word Count`; no `next:`)
- [ ] AC3 arity-9 + AC7 T286 pin + AC8 T241 bootstrap + AC9 T180 are **stay-green** (not Phase-1 red)

## Phase 2 — Green

- [ ] F7 rename label inside 9-arg formatter
- [ ] F2/F35 helper using `LIST_RECALL_QUERY`; F8 insert after budget-window line
...
- [ ] AC3 9-arg
- [ ] AC7 T286 tagged/untagged pin: `in_context_decisions >= 1` **and** T315 `next_step` omitted
...
- [ ] T286 pin hermetics stay green and omit T315 next
