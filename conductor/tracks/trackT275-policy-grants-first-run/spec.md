...
- **Research date:** 2026-08-21 (plan dogfood HEAD `8cb1ce0` T274 `#189`; product `src/` = T274). Fold-in against `c576b58` (plan docs; crates identical to `8cb1ce0`).
- **AI fold-in:** 2026-08-21 `agy-review.md` + `opencode-review.md`. **B 0 / M 0.** **Agree:** Agy m2 personal deny regression (F35 / AC16); Agy O1 AC4 principal-trap comment (F36); Agy O1/O2 const adjacency (F37). **Already:** Agy m1 ≤140 (F2 / AC2; frozen string **88** chars). **Fold snapshot:** OpenCode m1 HEAD `c576b58`; m2 clap Bootstrap `:2211`; m3 `empty_denied` `:218`. **Clarify:** OpenCode m4 JSON grant-wall (F3); m5 AC2 renderer-order only (F29); m6 F16 control-plane renderer. Disposition **§13**.
- **Ledger:** planning DOCS TX `e13a4e01-3dd6-4adc-ae57-be75e7e98ba9`. Fold-in DOCS TX `79f6e233-42ba-4660-b7c5-5560579ddece`. Implement starts a **FEATURE** TX on **go**.
...
| Signal | Observation |
|--------|-------------|
| HEAD | **Plan dogfood:** `8cb1ce0` T274 squash `#189`. **This fold-in:** `c576b58` (`docs(conductor): plan T275 grant-wall and CLI bootstrap unlock`). `origin/main` = `8cb1ce0`; local ahead by **1** (docs only). `git diff 8cb1ce0 HEAD -- crates/` empty — product `src/` identical to T274. Tree **CLEAN**. (OpenCode m1) |
...
| Denied project packet | `control-plane/.../briefings/project.rs` **`:216–229`** | `empty_denied` at **`:218`**; `denial_hint` at **`:224`**. (OpenCode m3 — plan `:217` was 1 line off) |
...
| clap Bootstrap | `main.rs` `PolicyCommands::Bootstrap` **`:2211`** (after_help `:2208`; dispatch `:4204`) | `--scope` optional, `--dry-run`/`-n`, `principal_id` env, format default json. **No new flags.** (OpenCode m2 — plan `:2207` was the after_help attr) |
...
| ID | Decision |
|----|----------|
| **F0 — Go gate** | Plan-only until user **go**. Planning is DOCS TX `e13a4e01`. Fold-in is DOCS TX `79f6e233`. Implement starts a **FEATURE** TX. |
...
| **F2 — Grant-wall sentence** | Frozen const `BRIEFING_DENIED_GRANT_WALL` = `This is a grant wall, not an empty vault. Pins remain via \`ai-brains recall\` / \`search\`.` **88** chars (Agy m1 — already ≤140). One line, after bootstrap next-step. Must contain `recall`. Must **not** replace bootstrap. AC2 asserts `.chars().count() <= 140` and no `\n`. |
| **F3 — JSON E1 freeze** | Denied JSON keeps `denied: true`, `decisions: []`, `conclusions: []` (not null, not omitted). `denial_hint` stays T241 short SOOT. **No** new DTO keys (`grant_wall`, `is_empty`, …). Machine honesty is **`denied: true`**. Do **not** extend `denial_hint` with grant-wall prose (T280 / OpenCode m4). Default non-TTY `briefing project` is JSON (`classify_briefing_format(None, false)`); human grant-wall is `--format human` / TTY markdown. |
...
| **F16 — No domain in CLI** | Grant-wall strings live in the **control-plane** `renderer.rs` (OpenCode m6 — not the CLI crate). Hermetic tests live in CLI. **No** `DefaultPolicyEvaluator` change. **No** grant logic in CLI beyond existing `run_bootstrap`. |
...
| **F21 — File growth** | Grant-wall consts + `_None_` branch in **`renderer.rs`** (F37 insertion point). New hermetic tests in `policy_bootstrap.rs` (or thin sibling; F36 comments). Renderer units AC1/AC2/AC6/AC16 next to existing denied tests. **Do not** grow `project.rs`, CLI `preflight.rs`, `doctor.rs`, `governed_common.rs`, `sync.rs`. |
| **F22 — Existing tests stay green** | T210 AC1–AC5; T221 progressive deny + after-bootstrap; T241 doctor/show/check; T263 granted-empty + Personal deny (`render_personal_markdown__denied__names_recall_not_personal_bootstrap` — AC16 extends) + list AC8; T227 substance; `render_project_markdown__denied__bootstrap_next_step_no_empty_authority` (bootstrap next still precedes Decisions). |
...
| **F28 — PowerShell** | `;` not `&&`. |
| **F29 — Determinism / budget** | Placeholder const frozen; renderer line order: Denied → bootstrap next → grant-wall → (optional blank) → Decisions hidden → Conclusions hidden. AC2 proves **renderer order** (grant-wall before `## Decisions`). Preflight word-budget survival is a **T227 analog, not a hermetic** (OpenCode m5) — do **not** grow CLI `preflight.rs` to assert it. Soft Phase-4 preflight budget test **declined as DoD**. |
...
| **F32 — Personal `_None_`** | Not audit 8/3. If the hidden-placeholder helper is shared, personal denied may use it; Personal next-step stays T263 recall. **Not required** to change Personal `_None_` if a separate branch is cleaner. Contamination **is** required (F35). |
...
| **F35 — Personal deny regression (Agy m2)** | `render_personal_markdown` denied path **must not** contain `BRIEFING_DENIED_GRANT_WALL`, `BRIEFING_DENIED_HIDDEN`, or `BRIEFING_DENIED_NEXT_STEP` / `policy bootstrap`. T263 unit `render_personal_markdown__denied__names_recall_not_personal_bootstrap` stays green; AC16 extends it with the new consts. |
| **F36 — AC4 comment (Agy O1)** | Hermetic AC4/AC5 in `policy_bootstrap.rs` **must** include a comment that `--principal-id` is omitted so bootstrap matches `cli_principal()` System default (T210 `bbbb…` trap). |
| **F37 — Const adjacency (Agy O2)** | Insert `BRIEFING_DENIED_GRANT_WALL` and `BRIEFING_DENIED_HIDDEN` immediately after `BRIEFING_DENIED_DENIAL_HINT` (before `BRIEFING_EMPTY_AUTHORITY_*`). Doc-comments distinguish deny-wall vs granted-empty vs Personal deny. |
...
| **AC2** | Frozen `BRIEFING_DENIED_GRANT_WALL` is **88** chars, `.chars().count() <= 140`, no `\n`; appears after bootstrap next and **before** `## Decisions` (**renderer order** — not a preflight budget hermetic; F29) | Unit |
...
| **AC16** | Personal denied markdown names `recall`, contains **no** `policy bootstrap` / `BRIEFING_DENIED_GRANT_WALL` / `BRIEFING_DENIED_HIDDEN` / `BRIEFING_DENIED_NEXT_STEP` (Agy m2 / F35) | Renderer unit (extend T263) |
...
Frozen hidden const `BRIEFING_DENIED_HIDDEN` = `_(hidden until discovery grants)_` (**33** chars). Dedicated const so AC6 allowed-empty `_None_` stays. F37: place both new consts immediately after `BRIEFING_DENIED_DENIAL_HINT`.

Do **not** omit the Decisions/Conclusions headings (preflight/agents scan headings). Replace the body only.

Default non-TTY `briefing project` is **JSON** (`format: Option`, `classify_briefing_format(None, false)`). Grant-wall is a **markdown** honesty fix. JSON agents already have `denied: true` (F3 / OpenCode m4).
...
Then green: F37 consts + denied `_None_` branch + AC16 personal (F35).

**Stay green:** AC6–AC14, T210, T221, T241, T263 (incl. personal deny unit), T227.
...
| Preflight word budget drops grant-wall | F29 renderer order; no preflight.rs growth (OpenCode m5) |
...
| Personal denied `_None_` | F32 optional; **F35 / AC16** contamination required |
| JSON grant-wall prose | F3 — `denied: true` is enough; T280 owns hint |
...
|------|-------------------|
| Renderer | `crates/ai-brains-control-plane/src/briefings/renderer.rs` — F37 consts + denied `_None_` branch + units AC1/AC2/AC6/AC16 |
...
## 13. AI fold-in (2026-08-21)

Sources: `agy-review.md` + `opencode-review.md`. **B 0 / M 0.** Review files are inputs — **do not edit**.

### Agy

| ID | Verdict | Action |
|----|---------|--------|
| **m1** GRANT_WALL ≤140 / one line | **Already** F2 / AC2 | Frozen literal **88** chars; AC2 `.chars().count() <= 140` + no `\n` |
| **m2** Personal markdown no project bootstrap | **Agree** | **F35 / AC16** — extend T263 unit with new consts |
| **O1** AC4 comment omit `--principal-id` | **Agree** | **F36** — required comment in `policy_bootstrap.rs` AC4/AC5 |
| **O2** Const adjacency | **Agree** | **F37** — GRANT_WALL + HIDDEN after `BRIEFING_DENIED_DENIAL_HINT` |

### OpenCode

| ID | Verdict | Action |
|----|---------|--------|
| **m1** HEAD `c576b58` vs `8cb1ce0` | **Agree snapshot** | §2.1; product crates identical |
| **m2** clap Bootstrap `:2211` not `:2207` | **Agree** | §2.3 citation |
| **m3** `empty_denied` `:218` not `:217` | **Agree** | §2.3 citation (`:216–229`) |
| **m4** JSON has no grant-wall prose | **Already F3** | Clarified: `denied: true` is machine honesty; do **not** extend `denial_hint` (T280). Default non-TTY is JSON. |
| **m5** Preflight budget analog | **Agree** | **F29** — AC2 is renderer order only; no preflight hermetic; do not grow `preflight.rs` |
| **m6** F16 “Domain in CLI” vs CP renderer | **Agree** | **F16** reworded — strings in control-plane `renderer.rs` |

### Declined / not new design

| Item | Why |
|------|-----|
| Extend JSON `denial_hint` with grant-wall | F3 / F11 — T280 owns hint wording |
| Preflight hermetic that grant-wall survives `-m` | F29 — analog only; CLI `preflight.rs` freeze |
| Auto-grant / `--install-grants` / live bootstrap | F8 / F9 / F10 unchanged |
| last-PR #189 Cursor | Still N/A empty; **no T285** |
| serde_json 1.0.151 / clap 4.6.6 / rusqlite 0.40.2 | **No bump** (F17); pin table snapshot updated |

### Pins locked by fold-in

1. `BRIEFING_DENIED_GRANT_WALL` exact string, **88** chars, ≤140 (Agy m1).
2. `BRIEFING_DENIED_HIDDEN` = `_(hidden until discovery grants)_` (**33** chars).
3. F35 / AC16 personal deny must not see project grant-wall consts.
4. F36 AC4/AC5 comment: omit `--principal-id` = System `cli_principal()`.
5. F37 const insertion point after `BRIEFING_DENIED_DENIAL_HINT`.
6. F16 strings in CP renderer, not CLI domain.
7. F29 AC2 = renderer order; no preflight.rs growth.
8. Citations: Bootstrap `:2211`; `empty_denied` `:218`; HEAD `c576b58`.
