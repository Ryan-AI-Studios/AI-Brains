...
- **Ledger:** planning DOCS TX `6bf1d41c-a2c6-4b86-8b4b-2dee14690363`. Implement starts a **FEATURE** TX on **go**.
...
| `memory list --summary` | `Pinned: 3822` / `Forgotten: 0` (volatile; T214 COUNT). |
...
| Count | `QueryStore::count_pinned_memories` `store/src/lib.rs:136` | T214. `None` = vault-wide; `Some` = project. **Use this for `Pinned: N`.** |
| Samples | `QueryStore::list_authority_memories` `:60` | T287. GLOB-or-TAGS on `mp.content`. **Reuse; no new store method.** |
| Preview | CLI `memory.rs` `preview_line` `:37` | T287 envelope. Import `pub(crate)`. Max **80**. |
| Classifier | `ranking.rs` `PinKind` `:65` — `Constraint` / `Decision` / `Hotspot` / `Other`. `INVARIANT:` → `Constraint`. | Preview retain **Decision \| Constraint** only. **Exclude Hotspot** (T279 Safety paths). **Do not edit ranking.rs.** |
...
| **F4 — Count is inventory, not GLOB** | `n` = `count_pinned_memories(Some(&project_id))` (T214). Includes harness ingest. **Do not** COUNT GLOB authority rows as `Pinned: N` (T287 R1-1 live GLOB 0). |
| **F5 — Previews are leading-line Decision/Constraint** | `list_authority_memories` (T287) with `status=pinned`, project scope, `tag=None`, `limit` ≥ 8 then retain `classify_pin_kind` ∈ `{Decision, Constraint}` (Invariant maps to Constraint). **Exclude Hotspot and Other.** Cap **3**. `preview_line(..., 80)` (T287 envelope). Empty after retain → human `_No leading-line DECISION/CONSTRAINT samples in this scope._` when `n > 0`; omit that line when `n == 0`. |
...
| **F11 — Renderer signature** | `render_project_markdown(packet)` **unchanged** (preflight + existing units). New `render_project_markdown_with_vault_pins(packet, Option<&VaultPinStanza>)`. `None` ≡ today’s render. CLI human uses the new fn. |
...
| **F14 — Scope from packet** | Parse `packet.scope.scope_key` `Repository:{uuid}` → `ProjectId`. Do **not** re-resolve cwd. `--project-id` already baked into the packet. Unparseable → fail-open. |
...
| **F20 — Tests** | Naming `function_or_feature__condition__expected_result`. `tempfile::tempdir` per hermetic. **AC14 required rstest `#[case]`** for overlay-gate (denied / nonempty / empty). |
...
| **F25 — Docs** | CAPABILITIES dual-model row: granted-empty CLI stanza + overlay keys; authority arrays still empty. CHANGELOG T288. `briefing project` after_help one sentence. OPERATIONS one-liner. PROTOCOL-COMPAT briefings row: CLI extras; daemon packet unchanged. |
| **F26 — PowerShell** | `;` not `&&`. |
...
| **AC12** | Manual (source/hermetic bin, not PATH): `cargo run -p ai-brains-cli -- briefing project --format human` contains `Pinned:` with nonzero (live ~3822 volatile) **and** `not Approved` **and** `recall`; Decisions stay `_None_`. JSON `denied: false`. Exit **0**. |
...
| **AC16** | Hotspot-only: `HOTSPOT: crates/foo.rs` pin → count ≥ 1, previews **omit** that line (F5 exclude Hotspot). |
...
2. FEATURE TX
3. Red AC1/AC2/AC3/AC14
4. Green: `VaultPinStanza` + render-with-pins + CLI collect/overlay
...
| `crates/ai-brains-control-plane/src/briefings/mod.rs` + `lib.rs` | Re-export new fn/type if CLI needs it |
