...
**Spec:** [spec.md](./spec.md) F0–F26 / AC1–AC12
**Category:** UX / FEATURE
...
| Check | Result |
|-------|--------|
| HEAD / tree | `ea5c947` T291 `#207` on `main`. CLEAN. `origin/main` = `ea5c947`. |
...
| `--help` | `[default: json]` |
| `preflight --summary` | Pinned **4020** (volatile); in-context **0/0/0**; word **267** |
...
| Hotspots | `project.rs` **#1** (3.941) — do not touch. `sync.rs` **#2** (3.628). `governed_common.rs` **#3** (3.613) — import consts only. `policy_cmd.rs` not top-10 — human + resolver here. CLI `preflight.rs` **#8** — do not grow. |
| Ledger | 0 pending / 0 drift at scan; planning TX `84c4b2ec` |
| `ISSUES.md` | **Does not exist** (F17) |
| ledgerful search | `run_check` → `policy_cmd.rs:140` / `main.rs:4331` |
...
- [ ] Confirm clap Check still `default_value = "json"` and **no** `value_parser`
...
- [ ] `policy_check__deny__format_human__denied_plus_short_exit_3` (AC3) — fail while stdout empty
- [ ] Commit red allowed
...
- [ ] AC2 allow-human hermetic (System bootstrap omit `--principal-id` on **temp** vault)
- [ ] AC4 json keys; AC6 clap `JSON` InvalidValue + default auto; AC7 pipe omit-format still JSON
...
- [ ] CAPABILITIES Family A row for `policy check`; OPERATIONS; CLI-EXIT-CODES; PROTOCOL-COMPAT §5; Check after_help; CHANGELOG
- [ ] AC9 hermetic `policy check --help` names auto/TTY
