...
- [ ] Re-dogfood: record **N** = `vault-*.db.bak` count (F38; plan-time 22); T244 still `(unreadable key)`? dry-run `--no-prune` size (F28 volatile); default keep still would prune M
- [ ] Confirm `src/help_ia.rs` + `tests/cli_help_ia.rs` still the F12 do-not-grow pair; AC5 stays in `backup_recoverable.rs`
...
- [ ] `backup_create_help__after_help__mentions_no_prune_default_dir` (AC5 / F37) — **must fail** before after_help exists: separate `--no-prune` / timestamp-not-class / `backups/` or `backup_recent` / `backup create --dry-run --no-prune`; no `--vault-path` (F35)
- [ ] Commit red allowed
...
- [ ] clap Create `after_help` (F6 / §5.1) — no new flags (AC6); examples include `--dry-run --no-prune`
- [ ] CAPABILITIES §11 + OPERATIONS Backup extend (Agy O1 list+doctor vs `--output-dir`) + CHANGELOG T295 (AC7)
- [ ] Optional RECOVERY-DRILLS one-liner
- [ ] Commit green allowed
- [ ] **Do not** edit brain `backup.rs` / CLI `backup.rs` production / `doctor.rs` / `src/help_ia.rs` / `tests/cli_help_ia.rs`
...
- [ ] `backup list --quiet` — new Readable first; **N+1** files (F38); paste transcript in `review.md`
- [ ] `backup verify` — ≥1 OK; exit 1 OK; **no** create nudge (Agy m2 / F14)
- [ ] `doctor --format json` — `backup_recent` not `no usable encrypted backup under current key`
- [ ] Residual files still present (**N** old + 1 new)
...
- [ ] `backup create --help` (no vault) separate locks: `--no-prune` + timestamp-not-class + `backups/` or `backup_recent` + `--dry-run --no-prune` example (F37)
...
- [ ] No `doctor.rs` / `project.rs` / engine / `src/help_ia.rs` / `tests/cli_help_ia.rs` production edit
