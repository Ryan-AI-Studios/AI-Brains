...
| **F27 — Helpers** | `pub fn class_dispose_count(b: &RetentionClassBucket) -> u64` in contracts (sum of the two u64s). **`pub` is required** — the helper lives in `ai-brains-contracts` and CP `audit_sample_ids` must call it; `pub(crate)` would be dead in the contracts lib. CLI Work iterates mechanisms. CP `pub(crate) fn audit_sample_ids(report: &RetentionPlanReport) -> Vec<String>` — crate-visible for same-crate units; **not** `pub` (integration `tests/` must keep using events — AC2). |
