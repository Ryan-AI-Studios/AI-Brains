...
- **Research date:** 2026-08-29 (plan-write product HEAD `464edc2` T319 `#236` conductor note; T319 product `#235` `e03c49d`). Snapshot — **re-verify at execute**.
- **AI fold-in:** none yet. Fold-in against this plan’s docs commit.
- **Ledger:** planning DOCS TX `dcb67912-8fb7-4bbd-a354-68ba41857744`. Series mint DOCS `a6d3c404-1d64-4cba-a743-d75ac16c74cd`. Implement starts a **FEATURE** TX on **go**.
- **Isolation:** Do **not** implement until **go**. Do **not** `cargo install`. Do **not** grow hotspot `project.rs` / `sync.rs` / `governed_common.rs` / `doctor.rs` / `daemon.rs` `run_status` / `graph.rs`. New sibling `commands/status.rs`. Dispatch **before** `AppContext` (doctor class). Do **not** print or commit `AI_BRAINS_KEY`. Do **not** start/stop the daemon, mutate schtasks, or `graph rebuild`.
...
| Signal | Observation |
|--------|-------------|
| HEAD | `464edc2` `chore(conductor): T319 Completed note with PR #235 squash e03c49d (#236)`. Tree **CLEAN**. Branch `track/T320-unified-status` off `origin/main`. Product `src/` = T319 `#235` `e03c49d` + conductor note. |
...
| `preflight --summary` (PATH) | Pinned **4563**. In-context **0/0/0**. `Total Word Count: 699` (PATH-behind T315 `Budget window words:`). **Not this DoD.** |
...
| `graph update` is feature-gated | Feature-off `graph *` exit **2**. Doctor `graph_density` is SQL in `graph_density.rs` (not feature-gated). Glance must work **graph-off**. |
...
| `graph_health_report` | `graph.rs:429` | **private**, feature-gated. **Do not import `graph.rs`.** |
| `gather_density_snapshot` / `assess_graph_density` | `graph_density.rs` | Shared by doctor + graph update. **Glance graph uses this.** Floors `MIN_EDGE_NODE_RATIO=0.50` `:14`. |
...
| Line counts (Measure-Object) | doctor **1738**; nightly **1968**; nightly_status **834**; daemon **1232**; graph **1606**; graph_density **668**; main **5402**; `governed_common` **1029**; project **1342**; sync **578** | Snapshot — F32 80-net is phase diff vs go HEAD. |
...
| **F11 — Graph section** | `gather_density_snapshot` + `assess_graph_density` in `status.rs` (or a `pub(crate)` one-liner in `graph_density.rs`). **Do not import `graph.rs`.** Feature-off still runs SQL. Human: `graph: {status}  nodes={} edges={} E/N={:.3} pinned={}`. JSON: `status`, `density`, `edge_node_ratio`, `nodes`, `edges` (no `note` / `remediation` — those stay doctor attention / `graph update`). Tables missing → `error`, not clap 2. |
| **F12 — Nightly section** | Vault `last_nightly_run` (`sync_state`) + Windows `fetch_schedule_snapshot("AI-Brains-Nightly")` (`scheduled` = `found && next_run.is_some()` matching nightly JSON `scheduled`; `last_task_result` from snap). Human: `nightly: last={rfc3339\|never}  scheduled={Yes\|No\|unknown}  last_result={code\|n/a}`. **No** Router, multi-import, unsummarized, Completion/Embedding. Non-Windows: `scheduled` JSON `null`, human `unknown` / `n/a`. |
...
| **F27 — next:** | Human last line + JSON `next_step` **only** when daemon Stopped. Exact `next: ai-brains daemon start` / JSON `ai-brains daemon start` (T249 string). Running → omit. Do **not** pile nightly/graph nexts. |
| **F28 — PowerShell** | `;` not `&&`. |
| **F29 — Isolation** | New `commands/status.rs` + `mod status`. Do not grow `project.rs` / `sync.rs` / `governed_common.rs` / `doctor.rs` / `daemon.rs` / `graph.rs`. `nightly.rs` **visibility-only** (`pub(crate)` fetch). `main.rs` clap + early dispatch + unreachable arm. `help_ia.rs` Daily/Start-here. |
...
| **F32 — Line-count 80-net** | `status.rs` new (no cap). Other production files: net &lt;80 vs **go HEAD** or split. `nightly.rs` visibility-only must stay ~0 net behavior. |
...
| **F45 — Tests** | Naming `function_or_feature__condition__expected_result`. No `unwrap`/`expect`/`panic` in production. No `std::env::set_var` in tests. |
...
| **AC6** | Unit: nightly never + Windows `found=false` → human `last=never` `scheduled=No`; JSON `last_run: null`, `scheduled: false`. |
| **AC7** | Unit: daemon Stopped → human last line exact `next: ai-brains daemon start`; JSON `next_step` exact `ai-brains daemon start`. Running → neither. |
...
| **AC9** | Hermetic: `ai-brains status --format json` on a temp vault exits **0**; stdout one JSON object; keys `schema_version`/`daemon`/`doctor`/`graph`/`nightly`. **Do not** assert host Running / live E/N / live last_run. |
...
| **AC13** | Isolation: `git diff` product — `doctor.rs` / `graph.rs` / `daemon.rs` `run_status` / `project.rs` / `sync.rs` / `governed_common.rs` / contracts **empty** of behavior. `nightly.rs` visibility-only. |
...
| `graph_density.rs` | Optional `pub(crate)` one-liner / `format_ratio` — **or** format in `status.rs` |
...
| doctor.rs / graph.rs / daemon.rs / contracts / retrieval | **No behavior** |
...
None yet (plan-write). Reviewers write `agy-review.md` / `opencode-review.md` only; fold-in applies agreed items.

**last-PR Cursor:** `#236` Bugbot overview / Low Risk — **not a finding**. `#235` empty. `#230` → T325. **No T326.**
