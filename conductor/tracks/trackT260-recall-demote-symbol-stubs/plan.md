...
**Status:** **In Progress** (implement-track go 2026-08-17)
...
- [x] Re-read `symbol_content` (`symbol_bridge.rs` ~595) and a live stub first line. Confirm format still `{kind} {qualified} ({path}:{line})` (or note drift and fix F1).
- [x] Re-read `recall_full` blend order and `candidate_depth`. Confirm still 15 at limit 5; filter must be SQL+memory (F7) not demote-only.
- [x] Re-read `RecallOptions` + every explicit literal (CLI, `sync.rs` ×2, `ai-brainsd`). Confirm T70 test still uses `Default`.
- [x] Re-read `lexical_search` / `forget.rs` / `substring_fallback` — still unfiltered default; F19 GLOB attaches only when `recall_full` asks.
- [x] Confirm SQLite `GLOB` + `[0-9]` still the exclude (F19); do **not** ship `LIKE` or `PRAGMA case_sensitive_like`.
- [x] Classify-only dogfood: `recall "what is this project" --no-bridge`; `--global "graph backend sqlite"`; `--semantic` capture-independence on path-owner. Confirm hole or note drift.
- [x] `--symbols --semantic` mix: if a hermetic decision+stub pair inverts, scale `SYMBOL_PENALTY` per `ScoreKind` (F9).
- [x] Re-check lock clap + crates.io: still no clap 5 (or this track is not that bump).
- [x] Rescan **entire** `conductor/deferred.md` for new open ranking / symbol-stub rows.
- [x] Last merged PR + open HEAD PR Cursor comments. Mint placeholder if a leftover fits nowhere.
- [x] `ledgerful ledger start T260-recall-demote-symbol-stubs --category FEATURE` (`d0ba0999-5ffd-4b65-af21-8eed8d23804e`)

---

## Phase 1 — Red (failing tests first)

- [x] Add retrieval units in `symbol_stub.rs` (or `ranking.rs` tests): AC1/AC2 detector names in spec §7.
- [x] Add `rerank_hits__included_symbol_below_decision__ac7` and `dedupe_symbol_stubs__identical_content_distinct_ids__one`.
- [x] Add `recall_full__default_excludes_symbol_stub__ac3` + `__symbols_includes_stub__ac4` + `__duplicate_symbol_content__deduped__ac6` + `__kind_prefix_non_locator__survives_default__ac16` + `__lowercase_module_locator__survives_default__ac17` (temp vault; pin DECISION + T70-format stub; no HTTP).
- [x] Add `lexical_search__default_still_returns_symbol__ac9`.
- [x] Add CLI hermetic `crates/ai-brains-cli/tests/recall_symbol_demote.rs` (tempdir + existing pin/init helpers; `TempEnv` + `#[serial(env)]` if overlapping keys).
- [x] Confirm Phase 1 fails for the right reason (stubs still win / flag missing). Commit allowed (red).

---

## Phase 2 — Green (detector + filter + flag)

- [x] New `crates/ai-brains-retrieval/src/symbol_stub.rs`: `SYMBOL_KINDS`, `is_symbol_stub_content` (fast `ends_with(')')`), **GLOB** `symbol_stub_sql_exclusion`, `dedupe_symbol_stubs`.
- [x] `ranking.rs`: `SYMBOL_PENALTY = 16.0` composite-space; apply when stub (F9). No second sort.
- [x] `RecallOptions.include_symbols` default false. Update every literal (F12).
- [x] `recall_full`: GLOB exclude + retain before RRF + after graph; **`rerank_hits` then F3 dedupe**; truncate.
- [x] Lexical/semantic/**substring**: apply GLOB helper **only** when caller asks (`include_symbols == false` from `recall_full`). Default `lexical_search` unfiltered (F10).
- [x] CLI: `--symbols` on Recall; plumb to options; pretty `[symbol]` (F4).
- [x] Flip T70 test to `include_symbols: true` (F13 / AC10).
- [x] `ai-brainsd` + `sync.rs` ×2 compile with `include_symbols: false`.
- [x] Targeted: `cargo nextest run -p ai-brains-retrieval -p ai-brains-cli --lib --bins` plus the new test file; `cargo clippy -p ai-brains-retrieval -p ai-brains-cli --all-targets -- -D warnings`.
- [x] Commit allowed (green).

---

## Phase 3 — Docs + honesty

- [x] CAPABILITIES Recall table: default exclude; `--symbols` mix; `[symbol]` pretty; no DTO (AC13).
- [x] CHANGELOG minor.
- [x] conductor row stays **Pending** until implement-track closeout; review.md on execute only.

---

## Phase 4 — Execute closeout (implement-track, not this pass)

- [x] Manual AC15 classify-only (no live mutate).
...
- [x] FEATURE TX commit; pin; review.md; FEATURE + codex-review; publish (PR + wait GHA + squash-merge + prune). **Never** `git push origin main`.

---

## Definition of done

- [x] AC1–AC17 checkable and green (AC15 recorded with exact commands)
- [x] F0–F19 honored; T240 F2 / T255 declines untouched
- [x] `forget --match` still finds stubs (AC9)
- [x] No `source_tag` migration; no clap 5; no new crates; no live `.env`
- [x] Medium+ review findings not silently dropped
- [x] CAPABILITIES + CHANGELOG updated
- [x] Ledger FEATURE TX committed; 0 pending 0 drift at close
