...
| Check | Result |
|-------|--------|
| HEAD / tree | `4088106` T264 `#179`. `main` = `origin/main`. CLEAN. |
...
- [ ] `list_paths__format_pretty__human_empty_copy` (AC3)
- [ ] `list_paths__format_json__api_version_1` (AC4) — extend existing json hermetic if needed
- [ ] `scan_roots__format_pretty__not_json` (AC5)
- [ ] Clap units in `main.rs` (AC2 / AC7): list-paths `pretty` parses; `xml` / `JSON` / `Pretty` InvalidValue; same for scan-roots; whoami/adopt-path/rebind-path `pretty` parses
...
- [ ] `project_paths.rs` / `project_adopt.rs` / `project_rebind.rs`: delete `use_json_output`; call `resolve_human_json_format` (F4 / AC6)
- [ ] `project.rs` whoami: replace format `match` only (F4). Do not touch `display_label` / detect / report struct
...
- [ ] CAPABILITIES OutputFormat table = Families A–D; nightly named Family B (F1 / F2 / AC11)
...
- [ ] AC1–AC13 pass with evidence
- [ ] F0–F26 honored (declines written)
