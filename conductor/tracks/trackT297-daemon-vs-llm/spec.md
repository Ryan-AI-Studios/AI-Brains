...
- **AI fold-in:** 2026-08-24 `agy-review.md` (`18ff6f7`) + `opencode-review.md` (`0132707` plan files). **Agy B 0 / M 0.** **OpenCode B 0 / M 1.** **Agree hard:** OpenCode M1 keep-bound `TcpListener` hermetic (rewrite F28/AC8). **Agree:** Agy m1 / OpenCode m3 rstest 8-tuple + F30 both-open; OpenCode m1 name-match capture; Agy O1 `Cli::try_parse_from`; Agy O2 T281 vs T297 docs; OpenCode O1 last-line in hermetic; OpenCode O2 T85 `:8080` stay-green. **Already:** Agy m2 U+2260 = F18/AC5; OpenCode m4 F20 already says `TCP connect`; OpenCode m2 conductor row already **Planned** on `18ff6f7`. **Decline:** OpenCode “fold-in cannot edit conductor.md”. Disposition **§13**.
- **Ledger:** planning DOCS TX `3f147d91-b4f9-42b2-a8c3-8ea01dd1292d`. Fold-in DOCS TX `338d0a5b-9dba-446a-8e07-89e0af81f610`. Implement starts a **BUGFIX** TX on **go**.
...
| **F7 — TCP probe freeze** | 5×100 ms + jitter **untouched**. Capture Open bools **by backend name** (F36). Do **not** extract `TcpStream::connect_timeout`. Do **not** add HTTP. |
...
| **F19 — Docs** | CAPABILITIES `:110` additive (contrast **before** last-line `next:`). OPERATIONS `:558` additive. **Required distinction (Agy O2):** nightly `--status` contrast is T281 `HTTP /health 750ms ≠ daemon TCP`; `daemon status` contrast is T297 `backend TCP Open ≠ daemon` (IPC vs model TCP). Root CHANGELOG T297 Unreleased. PROTOCOL-COMPAT **untouched**. CLI-EXIT-CODES status footnote already exit 0 both states — **no change required** unless a sentence is missing “Open is not Running”. |
| **F20 — after_help** | Exact `DaemonCommands::Status` `after_help` (one sentence, clap unit variant): `LLM/Embedding Open is TCP connect to the model process, not the AI-Brains daemon.` Reuse Nightly’s established phrase **`TCP connect`** (`main.rs` **`:1454`** `daemon status Open is TCP connect.`) — do **not** say `TCP probe` / `HTTP`. No new flags. |
...
| **F28 — Wiring hermetic (keep-bound listener)** | **Required** (OpenCode M1). Do **not** `daemon stop` to force Stopped (T249 AC7 / F11). Bind `TcpListener` on `127.0.0.1:0` and **keep it bound** for the whole `daemon status` (not T94’s drop-then-delay-rebind). Set `AI_BRAINS_MODEL_URL=http://127.0.0.1:{port}`. Exit **0**. **If** stdout `Status: Stopped` → **must** contain `BACKEND_OPEN_NE_DAEMON` **and** last non-empty line is `next: ai-brains daemon start`. **If** `Status: Running` → **must not** contain the const. CI/hermetic with no live daemon is Stopped+Open — that is the `run_status` wiring proof. Live Running degrades to the negative arm. |
...
| **F35 — Exhaustive bool matrix** | `status_backend_contrast_line` units are `rstest` `#[case]` over all **8** `(is_running, llm_open, embed_open)` triples (Agy m1). `Some` iff `!is_running && (llm_open \|\| embed_open)`. Must include `(false, true, true)` (F30). Do **not** for-loop inside one `#[test]`. |
| **F36 — Capture by name** | In `run_status`’s existing tuple loop, set bools by `name`: `"LLM backend"` → `llm_open`; `"Embedding backend"` → `embed_open` (OpenCode m1). Do **not** assign both bools from a single `state == "Open"`. Do **not** use index-only unless the names stay adjacent in the same match. |
...
| AC | Proof |
|----|-------|
| **AC1** | Unit `rstest` `#[case]`: all **8** triples of `status_backend_contrast_line` (F35). `Some(BACKEND_OPEN_NE_DAEMON)` iff `!is_running && (llm_open \|\| embed_open)`; else `None`. Must include `(false, true, true)` |
| **AC2** | Named arm of AC1: `(false, false, true)` → `Some` (embed-only Open) |
| **AC3** | Named arm of AC1: `(false, false, false)` → `None` |
| **AC4** | Named arm of AC1: `(true, true, true)` → `None` |
| **AC5** | Unit: const contains U+2260; `assert_ne!` vs ASCII `!=` string; length/content freeze `backend TCP Open ≠ daemon` |
| **AC6** | Unit: `status_report_tail(false, true, false) == [BACKEND_OPEN_NE_DAEMON, "next: ai-brains daemon start"]` (last is `next:`). `status_report_tail(false, false, false) == ["next: ai-brains daemon start"]`. `status_report_tail(true, true, true).is_empty()`. **F30:** `status_report_tail(false, true, true)` is **exactly two** lines: const then `next:` (one contrast, not two) — rstest `#[case]` over `(true,false)` / `(false,true)` / `(true,true)` for the Open pair |
| **AC7** | In-process (Agy O1): `Cli::try_parse_from(["ai-brains", "daemon", "status", "--help"])` → `Err` DisplayHelp; `err.to_string()` contains the F20 exact sentence tokens `TCP connect` and `model process` (same pattern as `nightly__help__names_router_267014_success` `main.rs` **`:956`**). Unknown extra flags still clap **2** |
| **AC8** | Hermetic keep-bound listener (F28 / OpenCode M1): `TcpListener::bind("127.0.0.1:0")` **held**; `AI_BRAINS_MODEL_URL=http://127.0.0.1:{port}`; `hermetic_bin_no_key` `daemon status`; exit **0**. Stopped → stdout contains const **and** `stdout.lines().filter(|l| !l.is_empty()).last() == Some("next: ai-brains daemon start")` (OpenCode O1). Running → stdout does **not** contain const. Never `daemon stop` |
...
`run_status`: `let mut llm_open = false; let mut embed_open = false;` then in the existing loop **after** `state` is known (F36):
...
Do **not** `llm_open = embed_open = state == "Open"`. After PID: `for line in status_report_tail(...) { println!("{line}"); }` — replace the current `if let Some(line) = status_next_line`.
...
### 5.5 AC8 keep-bound listener (not T94)

T94 (`smoke.rs` **`:3103–3146`**) binds `127.0.0.1:0`, **drops** it, then rebinds after 250 ms to prove retries. AC8 must **hold** the first listener for the whole `daemon status` so Open is immediate (no delay, no race with 5×100 ms). Same env-injection pattern (`AI_BRAINS_MODEL_URL=http://127.0.0.1:{port}`). Live `accept()` is not required — a listening socket is enough for `connect_timeout` to succeed.

T85 smokes (`smoke.rs` **`:2901–2999`**) set a dead `MODEL_URL` with embedding **unset** (default `:8080`). If a CI host ever has `:8080` Open and the daemon is Stopped, the contrast line appears. Those tests `contains("Closed")` on the **model** URL only — extra contrast is **allowed** (OpenCode O2). Do **not** add `!contains(BACKEND_OPEN_NE_DAEMON)` there.
...
1. `status_backend_contrast_line__matrix` (`rstest` 8 triples, F35) — includes `(false, true, true)`
2. `backend_open_ne_daemon__uses_u2260_not_ascii`
3. `status_report_tail__stopped_open_pair__single_contrast_then_next` (`rstest` F30 three Open pairs)
4. `status_report_tail__stopped_closed__next_only`
5. `status_report_tail__running__empty`
6. `daemon__help__status_names_backend_tcp` (`Cli::try_parse_from` DisplayHelp)
7. `daemon_status__keep_bound_listener__contrast_when_stopped` (AC8 — red until `run_status` wires tail)

**Then green:** const + helpers + **name-match** bool capture + tail print + after_help + keep-bound hermetic + docs.

**Stay-green:** AC9 / AC13 / AC14. T85 may gain a contrast line if `:8080` is Open — do not `!contains` the const there (§5.5).
...
| Risk | Mitigation |
|------|------------|
| Live daemon Running hides Stopped+Open | AC8 keep-bound listener: CI Stopped+Open is the wiring proof; Running degrades to const-absent. Units AC1/AC6 still SoT. Manual records Running. Do **not** stop the daemon. |
...
2. Red AC1 rstest + AC5–AC7 units + **AC8 keep-bound hermetic**.
3. Green const + helpers + **F36 name-match** + tail print + after_help.
4. Docs CAPABILITIES / OPERATIONS (T281 vs T297) / CHANGELOG.
5. Stay-green AC9/AC13/AC14 (no T85 `!contains` const).
...
| Live daemon Running (PID 4536) | Honest; AC8 Stopped+Open is CI/keep-bound; units lock F30 |
...
| Path | Change |
|------|--------|
| `crates/ai-brains-cli/src/commands/daemon.rs` | F1–F5 helpers + F35 rstest units; `run_status` **F36 name-match** bools + tail print |
| `crates/ai-brains-cli/src/main.rs` | `DaemonCommands::Status` exact F20 `after_help` + AC7 `try_parse_from` unit |
| `crates/ai-brains-cli/tests/daemon_status_vault_independence.rs` | AC8 **required** keep-bound listener hermetic (F28) |
...
Inputs (not edited): `agy-review.md` (HEAD `18ff6f7`) + `opencode-review.md` (HEAD `0132707` — plan files; product `src/` identical to `18ff6f7`). Fold-in on `main` at `18ff6f7`. Live verify: `run_status` **`:706–805`** still prints `status_next_line` only (`:801–803`); TCP loop **`:739–778`**; names `"LLM backend"` / `"Embedding backend"` **`:740–741`**; T94 **`:3103–3146`** drop-then-delay; T85 **`:2901–2916`** `contains("Closed")` not `!Open`; Nightly after_help **`:1454`** `daemon status Open is TCP connect.`; help unit pattern **`:956`**; conductor T297 row **already Planned** (`conductor.md` **`:244`**). Pins **snapshot — re-verify at execute** (clap lock 4.6.1 / crates.io 4.6.6; rusqlite 0.39.0; **no clap 5**). Last merged PR still **#212** (comments/reviews **empty**). **No T301.**

### Pins locked by fold-in

1. **F28 / AC8 (OpenCode M1):** keep-bound `TcpListener` hermetic is **required** wiring proof — not the vacuous if-Stopped-and-Open skip.
2. **F35 / AC1 (Agy m1):** rstest **8** triples; include `(false, true, true)`.
3. **F30 / AC6 (OpenCode m3):** both-Open tail is **exactly** `[const, next:]`.
4. **F36 / §5.2 (OpenCode m1):** capture by `name == "LLM backend"` / `"Embedding backend"`.
5. **F20 / AC7 (OpenCode m4 + Agy O1):** exact after_help uses **`TCP connect`**; in-process `try_parse_from`.
6. **F19 (Agy O2):** docs name T281 HTTP contrast vs T297 backend-TCP-vs-IPC contrast.
...
|--------|------|-------------|
| Agy | B / M | None filed |
| Agy | **m1** 8-permutation bool coverage | **Folded** F35 / AC1 rstest |
| Agy | **m2** U+2260 vs ASCII `!=` | **Already** F18 / AC5 |
| Agy | **m3** Running/Stopped hermetic skip | **Partial** — Running skip stays; Stopped+Open is **no longer optional** (OpenCode M1 / F28) |
| Agy | **O1** in-process clap `--help` | **Folded** AC7 `Cli::try_parse_from` |
| Agy | **O2** CAPABILITIES/OPERATIONS T281 vs T297 | **Folded** F19 |
| OpenCode | B | None filed |
| OpenCode | **M1** AC8 vacuous; no `run_status` wiring proof | **Folded** F28 / AC8 keep-bound listener + last-line `next:` |
| OpenCode | **m1** bool-capture discrimination | **Folded** F36 / §5.2 `match name` |
| OpenCode | **m2** conductor.md still Placeholder | **Already** — `18ff6f7` row is **Planned**; OpenCode reviewed `0132707` |
| OpenCode | **m3** F30 no red test | **Folded** AC6 rstest both-Open |
| OpenCode | **m4** after_help `TCP connect` vs Nightly | **Already** F20 phrase; **tightened** exact sentence |
| OpenCode | **O1** hermetic last line `next:` | **Folded** AC8 |
| OpenCode | **O2** T85 `:8080` stay-green | **Folded** §5.5 / F25 note |
| OpenCode | “fold-in cannot edit conductor.md” | **Decline** — fold-in skill allows `conductor.md`; row already Planned |
| both | last-PR #212 Cursor | **Affirm F14** — no T301 |
| both | deferred T298–T300 / T240 F2 / 750 ms / HTTP / JSON | **Affirm** |

No Blockers. One Major folded (M1). No new placeholder minted. Do **not** edit `*-review.md`.
