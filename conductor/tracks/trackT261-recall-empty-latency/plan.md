...
**Spec:** [spec.md](./spec.md) F0–F18 / AC1–AC18
...
| Check | Result |
|-------|--------|
| HEAD / tree | `1842df0` T260 `#175`. CLEAN. `main` == `origin/main`. |
...
| Research | ES empty→match-all footgun (NEST #2179); `match_none` pattern. sqlite.org FTS5 MATCH is token-based; LIKE `% %` is match-all. clap 4 accepts `""`. |
...
- [ ] Re-read `contentful_tokens` / stopword list / `len < 2`. Confirm F1 still matches T217.
