...
**Status:** **T256 Completed** 2026-08-16. **T257 Completed** 2026-08-17. **T258 Completed** 2026-08-16. **T259 Completed** 2026-08-17. **T260 Completed** 2026-08-17. **T261 Completed** 2026-08-17. **T262 Completed** 2026-08-17. **T263 Completed** 2026-08-18 (H1 only; H2 declined). **T264 Completed** 2026-08-18 (label+cap+span; leftover recall drop declined). **T266 Completed** 2026-08-18 (inventory tokens + four-family table; auto default stays). **T267 Completed** 2026-08-18 (harness `none` + list footer; AC7/AC16 leftover path-count; whoami affirm T258). **T265 Completed** 2026-08-19 (additive `sections[]`; F5 match + F6 session one-item; json-v2 declined). **T271 Completed 2026-08-19** (FTS-quote lift + token rescue; F18/F19 units; F2 Windows-only). **T268 Completed 2026-08-19** (`--root` XOR; empty registered suggested; parent hint). **T269 Completed 2026-08-20** (Nightly heading + timeout budget label; 750 ms not raised; #185 Cursor N/A). **T270 Planned 2026-08-20** (memory_legacy COUNT overlay; pinned→held; `Nothing to dispose.` = no dispose work; fold-in agy+opencode sample fallback + class sort + notes const; #187 Cursor N/A; still Pending until go). **T272 Completed 2026-08-20** (post-cap `safety_ids`; #187). **T273 Completed 2026-08-20** (POSIX `--` on `ledger search` argv; minted from #183; #184 Linux units declined as already fixed).
...
| `preflight --format json` is a `{text, word_count}` blob | **7/6** | **T265 Completed** | P2 |
| Format policy maze (human vs JSON default; list-paths JSON wall; retention default JSON on non-TTY) | friction; list-paths **7/5**; retention plan **6/5** | **T266 Completed** | P1 |
| Next-action is the command you just ran (`harness status`; `whoami` → whoami); `project list` suggests aliasing `7d97a456` as `AI-Brains` | harness **8/6**; list **8/6**; opp: real remediator | **T267 Completed** | P1 |
| `project scan-roots` only scans cwd; suggests re-registering an existing path | **4/5** | **T268 Completed** | P2 |
| Nightly human mixes Nightly Last Result **0** with Router **267009**; full status completion `probe=timeout` while daemon says Open | friction (nightly scores ≥7) | **T269 Completed** | P2 |
| `retention plan` 0 candidates across 35,300 memories (`memory_legacy / none_auto`) | **6/5** | **T270 Planned 2026-08-20** — COUNT overlay; live 38,208 pinned still `skip 0` until go | P2 |
| `sync query` ledger pane empty for a query this repo’s ledger should hit | **5/5**; friction | **T271 Completed** | P1 |
| Dash-leading `sync query` parsed as Ledgerful `--limit`/`--days` (Cursor #183) | Bugbot Medium after T271 | **T273 Completed** (minted T268 last-PR pass) | P2 |
...
- New track after T271 in the **original 2026-08-16 registration**. **T272** is a later leftover mint (Cursor #179 on T264). **T273** is a later leftover mint (Cursor #183 on T271). Neither is an audit-map remint.
