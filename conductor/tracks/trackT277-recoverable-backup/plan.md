...
**Spec:** [spec.md](./spec.md) F0–F41 / AC1–AC15
...
| Check | Result |
|-------|--------|
| HEAD / tree | `5ece8d5` T276 `#191`. CLEAN; `main` = `origin/main` |
| PATH `ai-brains` | **0.1.1** mtime 2026-08-21 05:55. **T270** on PATH. Backup classify is T244-era. **Do not `cargo install`.** |
| `preflight --summary` | Pinned **3376**; in-context 0/0/0; grants **0 of 3**; Scope `3581317d` |
...
| `backup create --dry-run --no-prune` | Would write `C:\dev\ai-brains\backups\vault-<now>.db.bak`; source **122433536** bytes |
...
- [ ] Re-read `run_backup_from_conn` (`backup.rs` ~`:146–218`), `classify_backup_read` ~`:440`, `is_usable_class` ~`:33`
...
- [ ] Brain unit `run_backup_from_conn__missing_cores__fails_and_deletes` (AC1) — **required red** (today create succeeds as Incomplete)
...
- [ ] `run_backup_from_conn`: after meta, `classify_backup_read`; `!is_usable_class` → remove_file + Err (F2/F34)
...
- [ ] AC1 red-then-green (fail-closed Incomplete create)
