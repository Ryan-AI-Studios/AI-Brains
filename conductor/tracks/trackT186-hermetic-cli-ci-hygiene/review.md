...
|----|-----------|--------|----------|
| **AC0** | Discoverable `.config/nextest.toml`; profile `ci` loads; valid slow-timeout terminate syntax | **Met** | Config at `.config/nextest.toml` with `slow-timeout = { period = "30s", terminate-after = 1 }` (correct nextest 0.9.x table form; not the invalid `terminate-after-slow-timeout` key). Root `nextest.toml` gone. `[profile.ci]` has `retries = 3`, `fail-fast = false`. Orchestrator ran suites with `--profile ci` successfully (profile discovery works). Docs document prove-load commands. |
