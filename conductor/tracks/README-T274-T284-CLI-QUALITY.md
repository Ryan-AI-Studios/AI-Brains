...
| Preflight Safety ≠ `safety sync --dry-run` hotspots | friction / opp | **T279 Planned** | P1 |
...
2. Next: **T279 Planned** — `/implement-track` / **go** (Safety vs hotspots). Then `/plan-track 280`
...
Do **not** `/implement-track` a Placeholder. Run `/plan-track TNN` first. T279 is **Planned** — **go** / `/implement-track` is the next execute step.
