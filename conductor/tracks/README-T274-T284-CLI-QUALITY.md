...
**Status:** All **Pending / Placeholder**. Full F-list only on `/plan-track TNN`. F0 = plan-only until **go**.
...
|---------|--------------|-------|-----|
| `recall` / `search` / `--semantic` rank review-track chats over pins; unique T270 DECISION sentence not in top-3 | 10/**4**, 10/**4**, 8/**4** | **T274** | P0 |
| `preflight --pretty` Safety/Session/Index are session dumps; `--summary` 0 decisions/hotspots/constraints | 9/**4**, 9/**5** | **T274** | P0 |
| `memory list --limit 5` is “just now” ingest, not pins; `sync query` vault half same dumps | 8/**7**, 9/**7** | **T274** | P0 |
