...
| `context --show` misses leftover shell vs `.env` (whoami has it) | 7/**7** | **T282 Completed** | P2 |
| `project list` leads with leftover 18k; cwd owner not first | 7/**6** | **T283 Completed** | P2 |
...
1. **T274 Completed.** **T275 Completed.** **T276 Completed.** **T277 Completed** (`#192`). **T284 Completed** (`#193`). **T278 Completed** (`#194`). **T279 Completed** (`#195`). **T280 Completed** (`#196`). **T281 Completed** (`#197`). **T282 Completed** (`#198`). **T283 Completed.**
2. Quality series T274–T284 closed. Soft residuals: PATH until `cargo install`; live leftover 11-root rebind (T276 F9); live `policy bootstrap` (T275 F10); live `backup create --no-prune` (T277); live `graph rebuild` (T278).
3. **T277** live `backup create --no-prune` still owner-confirm (hermetic shipped)
4. **T283** list cwd-first shipped (human promote; JSON frozen)
5. **T282** leftover `--show` shipped `#198` (no T240 F2; KEY redact)

Do **not** `/implement-track` until the owner says **go**.
