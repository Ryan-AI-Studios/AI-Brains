...
| `context --show` misses leftover shell vs `.env` (whoami has it) | 7/**7** | **T282 Planned** | P2 |
...
1. **T274 Completed.** **T275 Completed.** **T276 Completed.** **T277 Completed** (`#192`). **T284 Completed** (`#193`). **T278 Completed** (`#194`). **T279 Completed** (`#195`). **T280 Completed** (`#196`). **T281 Completed** (`#197`). **T282 Planned.**
2. Next: **T282** needs owner **go** (`/implement-track`). **T283** still needs `/plan-track` first. Do not `/implement-track` a Placeholder.
3. **T277** live `backup create --no-prune` still owner-confirm (hermetic shipped)
4. **T282** leftover `--show` Planned; **T283** list cwd-first still Placeholder
5. **T281** nightly timeout next-line shipped (do not raise 750)

Do **not** `/implement-track` a Placeholder. Run `/plan-track TNN` first. T283 still needs `/plan-track`.
