...
| Identity warn on every command / JSON interleave (`scope` 6/5, json 7/6) | **T257 Planned + fold-in 2026-08-17.** Spec F0–F26 / AC1–AC17. Live: split streams parse; `2>&1` is JSON-then-Warning. JSON-effective suppress + scope token. Both `scope.rs` emit sites (AC17). T240 list hermetic stands. F0 until go. |
