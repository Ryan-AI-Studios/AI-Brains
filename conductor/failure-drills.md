...
| Drill ID | Scenario | Expected Result | Verification |
|---|---|---|---|
| F-REC-01 | Delete active vault | Events recovered from most recent encrypted backup. | **T181-R-01** automated (seed → backup → force restore → content). Manual: delete `vault.db`, `ai-brains backup restore --force`, verify content. See [RECOVERY-DRILLS.md](../Docs/RECOVERY-DRILLS.md). |
| F-REC-02 | Restore on new machine | Recovery kit unlocks vault on a fresh host (library path today). | **T181-K-01..K-06** automated library drills. **No** `recovery export` CLI yet — operator must store kit out-of-band at init (RECOVERY-DRILLS §4). |
