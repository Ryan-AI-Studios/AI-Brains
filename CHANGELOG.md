...
- **T201 BREAKING — missing required `--scope`:** `policy show`, `review list`, and `erasure request` now require `--scope` via clap (`String`). Forgetting the flag exits **2** (clap English usage on **stderr**) instead of **6** with JSON `INVALID_PAYLOAD` on **stdout**. Pass `--scope` explicitly. Daemon/HTTP raw paths may still return `INVALID_PAYLOAD` (6) if scope is omitted (defensive arms retained). Soft: explicit `INVALID_TRANSITION` → exit **1** map arm.
