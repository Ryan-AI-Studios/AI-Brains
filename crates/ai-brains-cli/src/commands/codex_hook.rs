//! `ai-brains codex-hook` — live UPS/Stop capture from Codex CLI (T253).
//!
//! Ingests payload `prompt` / `lastAssistantMessage` directly after T234 filter.
//! Does **not** parse `transcript_path`. Missing fields: exit 0, no ingest.

use crate::context::{AppContext, StoreSink};
use ai_brains_adapters::codex::{
    CODEX_HARNESS_UUID, CODEX_UNBOUND_ALIAS, CODEX_UNBOUND_DISPLAY_NAME, accept_codex_live_payload,
    codex_env_fallback_allowed, generate_codex_live_turn_id, normalize_codex_project_hash,
...
pub fn run(ctx: &AppContext, payload_json: &str) -> Result<(), Box<dyn std::error::Error>> {
    let mapped = match accept_codex_live_payload(payload_json) {
        Ok(Some(m)) => m,
        Ok(None) => {
            eprintln!("[ai-brains-codex] skip: unrecognized hook stdin");
            return Ok(());
        }
        Err(e) => return Err(e.into()),
    };

    let session_id = session_id_from_codex(&mapped.session_id);
    let codex_harness = HarnessId::from_str(CODEX_HARNESS_UUID)?;

    let raw_hash = mapped.project_hash.trim();
    let alias_key = normalize_codex_project_hash(raw_hash);
    ...
    if project_id.is_none()
        && codex_env_fallback_allowed(raw_hash)
    ...
    let project_id = match project_id {
        Some(pid) => pid,
        None if alias_key == CODEX_UNBOUND_ALIAS => {
            ensure_or_create_project(ctx, CODEX_UNBOUND_ALIAS, CODEX_UNBOUND_DISPLAY_NAME)?
        }
        None => {
    ...
    if planned.is_empty() {
        eprintln!(
            "[ai-brains-codex] skip: empty prompt/lastAssistantMessage after message-only filter"
        );
        return Ok(());
    }
    ...
    if !used_env_fallback {
        ...
        eprintln!(
            "[ai-brains-codex] unbound session routed to env project {project_id} (alias not stamped)"
        );
    }

    eprintln!("[ai-brains-codex] event={}", mapped.event);

    let stable = mapped
        .turn_id
        .as_deref()
        .or(mapped.uuid.as_deref())
        .map(str::trim)
        .filter(|s| !s.is_empty());
    ...
    for (role, content) in planned {
        ...
        let turn_id = generate_codex_live_turn_id(&session_id, &mapped.event, stable, role_suffix);
        ...
        if stable.is_some() {
            ...
            match skip_on_query_error(query_store.get_sync_state(&key), "ai-brains-codex") {
                None => return Ok(()),
                Some(Some(_)) => {
                    eprintln!("[ai-brains-codex] skip existing {role} turn");
                    continue;
                }
                Some(None) => {}
            }
    ...
    if let Some(err) = sink.last_error {
        return Err(format!("Codex hook ingest error: {err}").into());
    }
    eprintln!("Successfully ingested {turn_count} turn(s) from Codex hook payload.");
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn codex_hook__query_error__skip_not_empty_vault() {
        assert!(
            skip_on_query_error(Result::<(), &str>::Err("locked"), "ai-brains-codex").is_none()
        );
        assert_eq!(
            skip_on_query_error(Result::<i32, &str>::Ok(1), "ai-brains-codex"),
            Some(1)
        );
    }
}
