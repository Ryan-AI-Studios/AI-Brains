...
#[test]
fn decision_in_force__omitted_term__fail_usage_exit_2() {
    let dir = tempdir().unwrap();
    let vault = dir.path().join("vault.db");
    init_vault(&vault);
    let scope = format!("Repository:{PROJECT}");
...
fn decision_in_force__term_flag_no_value__fail_usage_exit_2(#[case] term_args: Vec<&str>) {
    let dir = tempdir().unwrap();
    let vault = dir.path().join("vault.db");
    init_vault(&vault);
    let scope = format!("Repository:{PROJECT}");
...
#[test]
fn decision_in_force__term_flag_equals_empty__fail_usage_exit_2() {
    let dir = tempdir().unwrap();
    let vault = dir.path().join("vault.db");
    init_vault(&vault);
    let scope = format!("Repository:{PROJECT}");
...
#[test]
fn decision_in_force__term_flag_workspace_id__format_nope__clap_exit_2() {
    let out = common::hermetic_bin()
        .arg("decision")
        .arg("in-force")
        .arg("--term")
...
#[test]
fn decision_in_force__positional_and_term_flag__clap_conflict_exit_2() {
    let out = common::hermetic_bin()
        .arg("decision")
        .arg("in-force")
        .arg("workspace_id")
