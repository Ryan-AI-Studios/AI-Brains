...
fn test_projector_creates_nodes_and_edges() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Check Project
    let mut res = conn.query(&format!("MATCH (p:Project {{id: '{}'}}) RETURN p.name", project_id))?;
    assert!(res.has_next());
    assert_eq!(res.get_next()?.get_column(0).map(|v| v.to_string()), Some("test-project".to_string()));
    ...
    assert!(res.has_next());
    assert_eq!(res.get_next()?.get_column(0).map(|v| v.to_string()), Some("1".to_string()));
