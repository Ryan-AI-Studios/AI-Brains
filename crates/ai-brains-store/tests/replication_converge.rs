...
//!
//! Harness lives in `common::twin_vaults` (shared with T178 security suite).
...
use ai_brains_sync::{
    AdversarialRelay, ContentTypeCode, ControlPayload, DeviceEnrolledPayload, EnvelopeId,
    ErasureAckPayload, GapSkipAuditPayload, MemoryFakeRelay, REPLICATION_SCHEMA_VERSION, RelayPort,
    SyncError, build_and_sign_control, enrollment_package, fingerprint_sha256,
    generate_device_keys, nil_content_key_id, sign_envelope,
};
use common::twin_vaults::{
    CREATED_AT, TestVault, TwinVaults, domain_event_count, domain_pin_plaintext, twins_ready,
};
use std::sync::Arc;
use uuid::Uuid;
