...
fn test_api_success_shape() {
    ...
    let expected = r#"{"ok":true,"data":{"foo":"bar"}}"#;
...
fn test_api_error_shape() {
    ...
    let expected = r#"{"ok":false,"error":{"code":"ERR001","message":"Something went wrong"}}"#;
    assert_eq!(serialized, expected);
}
...
fn test_api_warnings_shape() {
    ...
    let expected = r#"{"ok":true,"data":{},"warnings":["Warning 1"]}"#;
    assert_eq!(serialized, expected);
}
