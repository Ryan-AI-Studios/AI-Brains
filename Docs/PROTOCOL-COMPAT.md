...
|----------------|-------|-------|
| `recall` JSON | **compact** (`to_string`) | Additive T218 (optional, default+skip): per-result `score_kind` (`bm25`\|`rrf`\|`bridge` only) + `cosine`; existing `score` / `source` / `staleness` / response `embedding` unchanged. T274 adds **no** new required keys (`is_session` / `pin_kind` are not on the wire). N−1 clients ignore unknowns. |
