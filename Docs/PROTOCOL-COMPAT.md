...
| `project scan-roots` | TTY **pretty** human; **pretty** JSON (`to_string_pretty`) when piped or `--format json` | T266/T254. Default `auto`. **keys unchanged**: `api_version`, `scan_root`, `truncated`, `roots[{path,registered_project_id,exists,suggested}]`. Same token map as list-paths. Human is **not** a wire contract. |
