...
| `project scan-roots` | TTY **pretty** human; **pretty** JSON (`to_string_pretty`) when piped or `--format json` | T266/T254/T268. Default `auto`. **keys unchanged**: `api_version`, `scan_root`, `truncated`, `roots[{path,registered_project_id,exists,suggested}]`. T268: `suggested` is `""` (not `null`) when `registered_project_id` is set. No `next_step` / `hint` key. Same token map as list-paths. Human is **not** a wire contract. |
