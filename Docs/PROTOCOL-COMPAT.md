...
| `graph` neighbors/hierarchy/session | TTY **pretty** human; **compact** JSON when piped or `--format json` | **keys unchanged**. T246 adds `--format` so the TTY/pipe split is gated (changing compact↔pretty without a flag remains breaking). T262 pretty next-action is **human-only** (rebuild iff vault has the id; unknown / leaf / empty edges have no `next:`). T278 session PREVIEW `{n} memories · first line` is **human-only** (not a JSON field). JSON still `{memory_id,neighbors}` / `{root,synthesized_from}` / `{session_id,memories}` with empty arrays on miss. |
| Array order (`graph` neighbors/hierarchy/session) | sorted (T246) | Array order: sorted for determinism (neighbors: direction→label→id; hierarchy/session: lexicographic). Pre-T246: SQL encounter order. T293: **pretty/human neighbors** may prefer-fill authority 1-hop first (human-only); **JSON** neighbors array order stays direction→label→id. |
