...
| `preflight` JSON (`--format json`, non-summary) | **compact** (`to_string`) | Required keys: `text`, `word_count` (T180). Additive T265: always-present `sections` array (`{id,title,items}[]`; E1 `[]`). Compact unchanged. N−1 2-key files deserialize with `sections: []`. T279 changes Safety **text** (live hotspot lines / honest empty); **T286** / **T327** / **T329** change Index / Session / Recent **item strings** (envelope titles; authority-first order; Session may include `+K more session turns via recall`; Index may include the F4 honesty line; Recent may omit a whale body that does not fit leftover budget); **no** new required keys (`hotspots[]` / `index_kind` / `envelope` are not on the wire). |
