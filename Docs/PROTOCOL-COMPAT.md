...
| `query expand` | **pretty** (`emit_json`) | **T314 (new row):** default `--format json`. **DTO keys unchanged** (`HandlePreviewDto`): `api_version`, `handle_id`, `kind`, `preview`, `truncated`, optional `source_version_id`. CLI JSON overlay adds `applied_scope` (scope identity key; not on the DTO / daemon `EvidencePreview`). Tokens `auto\|pretty\|human\|text\|json\|markdown\|md` (case-sensitive; `JSON`/`Pretty` exit **2**). Omitted `--format` stays JSON (no default TTY flip); `--format auto` is TTY human / pipe JSON. Human/`pretty`/`text`/`markdown`/`md`: `kind` then `preview` — human is **not** a wire contract. Denied JSON may keep empty `preview`; human Denied fills a nonempty second line (`Access denied.`) so Unknown/Denied are two nonempty lines. Unknown exit **0**; Denied exit **3**. |
