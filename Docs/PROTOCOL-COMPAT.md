...
|----------------|-------|-------|
| `recall` JSON | **compact** (`to_string`) | Additive T218 (optional, default+skip): per-result `score_kind` (`bm25`\|`rrf`\|`bridge` only) + `cosine`; existing `score` / `source` / `staleness` / response `embedding` unchanged. **T361** additive omit-if-none on empty results: `empty_kind` closed set `empty_scope`\|`query_miss`\|`scope_unowned`, plus `project_memory_count` (u64 including 0). Both omitted when `results` nonempty and on `--global`. T274 / **T285** / **T312** add **no** new required keys (`is_session` / `pin_kind` / `envelope` / `verbose_dump` are not on the wire). N−1 clients ignore unknowns. |
