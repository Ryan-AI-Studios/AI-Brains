...
| `retention plan` | TTY **pretty** human; **pretty** JSON (`to_string_pretty` / `emit_json`) when piped or `--format json` | T248/T270. **keys unchanged**: `api_version`, `generated_at`, `mode`, `horizons`, `classes`, `totals`, `cascade`, `warnings`, `errors_count`, optional `errors`. Live vaults may emit a `memory_legacy` bucket (inventory overlay; still a normal `classes[]` row). Empty vault stays `classes: []`. Human path is **not** a wire contract. `--format` tokens are **case-sensitive** (`JSON` / `Pretty` exit 2). |
