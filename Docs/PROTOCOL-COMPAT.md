...
| `graph` neighbors/hierarchy/session | TTY **pretty** human; **compact** JSON when piped or `--format json` | **keys unchanged**. T246 adds `--format` so the TTY/pipe split is gated (changing compact↔pretty without a flag remains breaking). T262 pretty next-action is **human-only** (rebuild iff vault has the id; unknown / leaf / empty edges have no `next:`). JSON still `{memory_id,neighbors}` / `{root,synthesized_from}` / `{session_id,memories}` with empty arrays on miss. |
