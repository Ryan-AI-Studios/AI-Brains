...
|----------------|-------|-------|
| `recall` JSON | **compact** (`to_string`) | Additive T218 (optional, default+skip): per-result `score_kind` (`bm25`\|`rrf`\|`bridge` only) + `cosine`; existing `score` / `source` / `staleness` / response `embedding` unchanged. **T361** additive omit-if-none on empty results: `empty_kind` closed set `empty_scope`\|`query_miss`\|`scope_unowned`, plus `project_memory_count` (u64 including 0). Both omitted when `results` nonempty and on `--global`. **T362** additive omit-if-none `fill_kind` closed `"index"` plus `hint` = `No FTS hits; showing in-scope pins` when any result `source` is `"index"` (N−1 ignore unknown `fill_kind`; `hint` was already optional). T274 / **T285** / **T312** add **no** new required keys (`is_session` / `pin_kind` / `envelope` / `verbose_dump` are not on the wire). N−1 clients ignore unknowns. |
| `preflight` JSON (`--format json`, non-summary) | **compact** (`to_string`) | Required keys: `text`, `word_count` (T180). Additive T265: always-present `sections` array (`{id,title,items}[]`; E1 `[]`). Compact unchanged. N−1 2-key files deserialize with `sections: []`. T279 changes Safety **text** (live hotspot lines / honest empty); **T286** / **T327** / **T329** / **T330** change Index / Session / Recent **item strings** (envelope titles; authority-first order; short `DECISION:` pins may surface; chrome titles may vanish from numbered Index; Session `+K` counts Other in a 20-turn window; Index may include the F4 honesty line with zero numbered items; Recent may be a truncated snippet when nothing else packs); **no** new required keys (`hotspots[]` / `index_kind` / `envelope` are not on the wire). |
| `preflight --summary --format json` | **pretty** (`to_string_pretty`) | T220 CLI-local envelope: `api_version`, `scope` (`global`\|`project`\|`none`), `project_id`, optional `projects` (global only), `pinned`, `active_sessions`, `in_context_*`, `word_count` (full budget text). T264 additive optional `in_context_project_span` (global only; omitted under project/none). **T286** does not add keys (`in_context_authority` is not on the wire); `in_context_decisions` stays a `DECISION:` substring of the assembled window. Optional `next_step` (skip when absent) string sources (first match): **T345** unowned location `next: ai-brains context`; **or** **T241** bootstrap when discovery grants incomplete; **or** **T365** leftover `next: ai-brains context --show`; **or** **T361** project-scoped pinned-0 `next: ai-brains capture coverage`; **or** **T315** empty-decisions `next: ai-brains recall "what did we decide"` when `in_context_decisions == 0`. Optional `grants_status` remains T241-only. **No** new required keys. Summary path does **not** grow into `PreflightContextResponse` (no `sections` on this object). |
| `scope resolve --format json` | **pretty** (`to_string_pretty` via `emit_json`) | T249 TTY/pipe split — default `auto` is TTY human / pipe JSON. JSON **keys unchanged**: `api_version`, `scope`, `confidence`, `authoritative`, `evidence`, `warnings`, `alternatives`. T257: `warnings[]` may include the additive token `project_identity_mismatch env=<uuid> path=<uuid>` when daily Scope ≠ path owner (not a new key; E1 empty remains `[]`). T328: `warnings[]` may also include `project_identity_collision env=<uuid> detect=<uuid>` when env ≠ detect **and** path-alias is absent (not emitted when the mismatch token already applies; E1 empty remains `[]`). **T332:** `warnings[]` may also include `project_detect_env_fallback env=<uuid> slug=<slug>` when path-alias is absent, detect source is env, and a git slug is present (not emitted when the mismatch or collision token already applies; skip `--no-project-context` / `--global`; E1 empty remains `[]`). Human path is **not** a wire contract. `--format` tokens are **case-sensitive** (`JSON` / `Pretty` exit 2). |
| Governed mutations (briefing, erasure, …) | **pretty** (`emit_json`) | Machine-clean stdout. T280 changes POLICY_DENIED `details.hint` **string wording** (omit required `--scope …`); **no** new required keys. Briefing `denial_hint` stays optional string. |
| `ingest` success response | **compact** | `event_id`, `processed` |
| `graph` neighbors/hierarchy/session | TTY **pretty** human; **compact** JSON when piped or `--format json` | **keys unchanged**. T246 adds `--format` so the TTY/pipe split is gated (changing compact↔pretty without a flag remains breaking). T262 pretty next-action is **human-only** (rebuild iff vault has the id; unknown / empty edges have no `next:`). **T317:** pretty hierarchy **leaf** may print `next: ai-brains nightly --status` (still no graph update/rebuild; JSON leaf stays `synthesized_from: []`). T278 session PREVIEW `{n} memories · first line` is **human-only** (not a JSON field). **T317:** human neighbors may hide RECALLS beyond 3 behind `+N more RECALLS` (human-only; JSON lists all). JSON still `{memory_id,neighbors}` / `{root,synthesized_from}` / `{session_id,memories}` with empty arrays on miss. |
| Array order (`graph` neighbors/hierarchy/session) | sorted (T246) | Array order: sorted for determinism (neighbors: direction→label→id; hierarchy/session: lexicographic). Pre-T246: SQL encounter order. T293: **pretty/human neighbors** may prefer-fill authority 1-hop first (human-only); **JSON** neighbors array order stays direction→label→id. **T317:** pretty may also cap displayed `RECALLS` at 3 after prefer (human-only); JSON array still full uncapped 1-hop. |
...
| `graph rebuild --format json` | **pretty** | **Same keys** as `graph update` report (T300). Default rebuild format is **human** (labeled lines). Rebuild does **not** add `next_step` / `events_replayed` / `dry_run` fields. JSON `--dry-run` prints the health object only (human `[dry-run]` line is human-only). |
...
| `status` | TTY **pretty** human; **pretty** JSON (`to_string_pretty`) when piped or `--format json` | **T320 (new row):** Family A `--format auto` default. CLI-local envelope `schema_version: 1` — **not** a contracts DTO. Required section objects: `daemon`, `doctor`, `graph`, `nightly`. Optional `next_step` only when daemon Stopped (prefix-less `ai-brains daemon start`). Section `error` (string) replaces that section’s other keys on fail-open. Doctor slim keys: `status`, `vault_path`, `ok`/`warn`/`fail`/`skip`, `attention[{name,severity,message,remediation?}]` — not full `DoctorReport.checks`. Graph keys: `status`, `density`, `edge_node_ratio`, `nodes`, `edges` (no `note`/`remediation`). Nightly keys: `last_run`, `scheduled`, `last_task_result` (JSON null when absent; `scheduled` null on non-Windows). Tokens `auto\|pretty\|human\|text\|json\|markdown\|md` (case-sensitive; `JSON`/`Pretty` exit **2**). Human is **not** a wire contract. Does not replace doctor / nightly --status / daemon status / graph update. |
| `project list-paths` | TTY **pretty** human; **pretty** JSON (`to_string_pretty`) when piped or `--format json` | T266/T254. Default `auto`. **keys unchanged**: `api_version`, `paths[{project_id,label,alias,normalized_path,exists}]`. Tokens `auto\|pretty\|human\|text\|json\|markdown\|md` (case-sensitive; `JSON`/`Pretty` exit 2). `--format pretty` ≡ human table. Human is **not** a wire contract. |
| `project scan-roots` | TTY **pretty** human; **pretty** JSON (`to_string_pretty`) when piped or `--format json` | T266/T254/T268/T314. Default `auto`. **keys unchanged**: `api_version`, `scan_root`, `truncated`, `roots[{path,registered_project_id,exists,suggested}]`. T268: `suggested` is `""` (not `null`) when `registered_project_id` is set. No `next_step` / `hint` key. Same token map as list-paths. **T314:** `--dry-run` accepted as a no-op (already dry-run-only; no new keys). Human is **not** a wire contract. |
| `retention plan` | TTY **pretty** human; **pretty** JSON (`to_string_pretty` / `emit_json`) when piped or `--format json` | T248/T270/T284. **report keys unchanged**: `api_version`, `generated_at`, `mode`, `horizons`, `classes`, `totals`, `cascade`, `warnings`, `errors_count`, optional `errors`. Class bucket required keys stay `class`, `candidate_count`, `mechanism`, `sample_ids`, `notes`. Optional extras `would_ce_wipe` / `would_projection_delete` / `dispose_sample_ids` emit only when non-zero (absent = 0). Live vaults may emit a `memory_legacy` bucket (inventory overlay; still a normal `classes[]` row). Empty vault stays `classes: []`. Human path is **not** a wire contract. `--format` tokens are **case-sensitive** (`JSON` / `Pretty` exit 2). |
...
| `device status` | **human** only | Same roster as `device list` + this-machine + short honesty + **always** last-line `next: ai-brains replicate status` (empty and enrolled). No `--format` (unknown flags clap exit **2**). Not a wire contract. Not a JSON DTO. Additive T251/T298; **not** a compact↔pretty flip. |
...
| `nightly --status --format json` | **pretty** | CLI-local `NightlyStatusJson` `schema_version: 1` (not a contracts DTO). **T334 additive:** `multi_import` **ok** always includes `claude` / `codex` / `cursor` (plus `agy` / `grok` / `opencode`). `never` / `unreadable` remain `{ "status": "never"|"unreadable" }` only (omit source keys). `last_multi_import` `v` stays **1**; missing new keys deserialize as skipped `absent_pre_t334`. Default `--format` is **human**; pipes stay human. |
...
| `query expand` | **pretty** (`emit_json`) | **T314 (new row):** default `--format json`. **DTO keys unchanged** (`HandlePreviewDto`): `api_version`, `handle_id`, `kind`, `preview`, `truncated`, optional `source_version_id`. CLI JSON overlay adds `applied_scope` (scope identity key; not on the DTO / daemon `EvidencePreview`). Tokens `auto\|pretty\|human\|text\|json\|markdown\|md` (case-sensitive; `JSON`/`Pretty` exit **2**). Omitted `--format` stays JSON (no default TTY flip); `--format auto` is TTY human / pipe JSON. Human/`pretty`/`text`/`markdown`/`md`: `kind` then `preview` — human is **not** a wire contract. Denied JSON may keep empty `preview`; human Denied fills a nonempty second line (`Access denied.`) so Unknown/Denied are two nonempty lines. Unknown exit **0**; Denied exit **3**. **T319:** when Unknown UUID exists as vault `memory_id`, CLI overlay replaces preview with namespace SOOT and adds optional `next_step` (`ai-brains recall "what did we decide"`); human becomes three nonempty lines. Unknown-unknown keeps T263 `Handle not found.` / no `next_step`. DTO / daemon unaugmented. |
| `evidence show` | **pretty** (`emit_json`) | **T319 (new row):** default `--format json`. **DTO keys unchanged** (`HandlePreviewDto`). CLI overlay may add optional `next_step` when Unknown UUID is a vault `memory_id` (same SOOT as expand). No `applied_scope`. Unknown exit **0**. Daemon IPC unaugmented; CLI probes local vault. |
| `source show` | **pretty** (`emit_json`) | **T319 (new row):** miss stays `NOT_FOUND` exit **4**; `message` is `source {id}`. When UUID is a vault `memory_id`, additive `details.hint` names the namespace + recall next. Invalid id stays exit **6**. Daemon IPC unaugmented; CLI attaches hint on local + daemon Error path. |
| `policy check --format json` | **pretty** (`emit_json`) | **T292 default lift (check only):** clap default `auto` = TTY human / pipe JSON. Allow JSON **keys unchanged**: `allowed`, `principal_id`, `capability`, `scope`. Deny JSON stays one ApiError (`POLICY_DENIED` + `details.hint`). Human allow/deny lines are **not** a wire contract. Tokens **case-sensitive** (`JSON`/`Pretty` exit 2). `policy show` / `policy bootstrap` stay default `json` (Family D). Scripts that previously parsed TTY default JSON must pass `--format json`. |
