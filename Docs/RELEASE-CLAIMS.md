...
| `ai-brains recovery export` (kit to file; no kit JSON on stdout) | Offline kit without operator path still residual |
| `ai-brains doctor` read-only health CLI (T192 check matrix) | Offline kit without `--kit-path` still operator residual (**R-DOC-CLI** residual half); does not auto-remediate |
...
| Inventing doctor auto-remediation or default kit path search (doctor **is** shipped T192 as read-only) | **R-DOC-CLI** residual |
...
| **Offline recovery kit without `--kit-path`** | Doctor shipped (T192) but kit file check is skip unless path given; event ≠ offline file proof (**R-DOC-CLI** residual) |
| **`ai-brains recovery export` CLI** | **Shipped (T188)** — kit file + RECOVERY-DRILLS |
| **`ai-brains doctor` CLI** | **Shipped (T192)** — read-only; residual = offline kit without `--kit-path` |
...
|-------------|------------------|-------------|---------------|
| **R-12** | Path TOCTOU / openat / cap-std | **Implemented-with-residuals** (T190 + **T193** / ADR-0021) | TrustedBuiltin **vault open+list** hardened (T190): cap-std ambient root + component nofollow; handle-bound read; zero ambient `read_dir` on walk. **T193 write SOOT:** `CreateMode::{CreateNew,Replace}` under ambient parent Dir + leaf nofollow (never Windows `TRUNCATE`+`OPEN_REPARSE`); elevated **P0** `write_protected_artifact`, token load/write, `write_kit_file`; **P1** migrate report/manifest, shadow-manifest, dogfood/evaluate report writes. Handle-bound hardlink refuse (nlink). **Residuals:** soft-canonicalize (non-claim for TOCTOU); parent `create_dir_all` chain; backup dest tree pure-ambient create (parent reparse refuse only); ambient CLI long-tail (adapters/context/elevation/discovery/cozo_proxy); perfect all-API Windows TOCTOU non-claim; symlink soft-skip in CI. |
| **R-34.2** | DataKey rotation / wrap-nonce budget | **Implemented-with-residuals** | T189 / ADR-0020: `vault rotate-datakey` (export primary); residuals = multi-device per-device ceremony, offline old kits/backups, rekey opt-in crash residual, Windows drop→MoveFileEx micro-window (OS handle close required). Argon2 params in kit JSON closed by T194. |
...
| **R-DOC-CLI** | Doctor **shipped** (T192); residual = offline kit without `--kit-path` | Cited as residual honesty | Doctor does not invent kit path; event log ≠ offline file existence; export remains product. |
...
- Inventing doctor auto-remediation / default kit path search (doctor is real as of T192; residual = offline kit without `--kit-path`)
