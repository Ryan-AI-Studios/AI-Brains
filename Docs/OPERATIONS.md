...
- **Windows service (recommended for persistent daemon):** `daemon install` registers `AI-Brains-Daemon` as a Windows service running as `LocalSystem` in Session 0. The named-pipe security descriptor is SDDL `D:(A;;GA;;;SY)(A;;GA;;;BA)(A;;GA;;;IU)` — **Local System + Built-in Administrators + Interactive** (not World/Everyone). That grants Session 1 interactive CLI clients cross-session access while excluding null-session/Everyone. **Residual:** any interactive logon on a multi-user host can open the pipe; pipe traffic has no HTTP-style bearer (see SECURITY-LIMITS §7). Env vars (vault path, model URLs) are written to `%ProgramData%\AI-Brains\daemon.env` with a restrictive ACL (`SYSTEM:F` + `Administrators:F` only — same model as the nightly SYSTEM wrapper; T145). Requires an elevated PowerShell session.
