...
| `retention plan` | human | json | **A.** T248: `--format auto` (default). TTY human class matrix; pipe / `--format json` pretty JSON. Keys frozen. |
...
| Scan Ledgerful roots (T254/T266) | `project scan-roots [path]` — dry-run. Immediate children (plus scan root) that contain `.ledgerful`. Never appends events. Never writes `.env`. Never auto-registers. `.changeguard` alone is **not** a hit. Cap 200 children. Same `--format` token map as list-paths. JSON `{ api_version, scan_root, truncated, roots:[{path,registered_project_id,exists,suggested}] }`. |
