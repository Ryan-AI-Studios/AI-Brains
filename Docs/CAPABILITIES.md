...
| Whoami (T240/T266/T328) | `project whoami` — all identity signals: `effective_project_id`, `env_project_id` (post-dotenv; null under `--no-project-context`), `shell_project_id` (pre-dotenv when set/differs), `path_alias_project_id`, `detect_project_id`, `git_slug`, `git_toplevel`, `mismatch`, **`identity_collision`** (always a boolean; never omit / never `null`), `remediations[]`. `--format auto` (TTY human / pipe JSON); same T266 token map as list-paths. Does **not** rewrite `PROJECT_ID`. `mismatch` stays env vs path-alias (both Some). **T328:** when env ≠ detect and path-alias is absent, `identity_collision: true` and remediations name `register-path` first (then `adopt-path`); collision is honesty, exit **0**. On mismatch, remediations name `project adopt-path` (print-only / `--write-env --yes`) plus the exact `AI_BRAINS_PROJECT_ID=<path-id>` assignment — not `project list`, not “run whoami”. |
