...
| List all path aliases (T254/T259) | `project list-paths` — every registered root (not just `project list` first-path). Optional `--project <id\|alias>` and `--shared-only` (owner appears on ≥2 roots). Combined flags are an intersection. `--format auto` (TTY human / pipe JSON) / `human` / `json`. JSON `{ api_version:"1", paths:[{project_id,label,alias,normalized_path,exists}] }` ASC by path. Unfiltered empty: `No path aliases registered.` + `next: ai-brains project register-path <project_id\|alias> <path>` (exit **0**). Filtered empty: `No path aliases match.` / `paths: []` (exit **0**). Unknown `--project` dest exit **1**. |
...
| Whoami (T240) | `project whoami` — all identity signals: `effective_project_id`, `env_project_id` (post-dotenv; null under `--no-project-context`), `shell_project_id` (pre-dotenv when set/differs), `path_alias_project_id`, `detect_project_id`, `git_slug`, `git_toplevel`, `mismatch`, `remediations[]`. Human default on TTY (`IsTerminal`); JSON when piped or `--format json`. Does **not** rewrite `PROJECT_ID`. On mismatch, remediations name `project adopt-path` (print-only / `--write-env --yes`) plus the exact `AI_BRAINS_PROJECT_ID=<path-id>` assignment — not `project list`, not “run whoami”. |
| Adopt path (T258) | `project adopt-path` — bind daily Scope to the path-alias owner of cwd / git toplevel. Default **print-only** (names owner + exact `AI_BRAINS_PROJECT_ID=<id>` assignment; no file write). `--write-env --yes` rewrites **only** that key in cwd `.env` (creates the file if missing). `--write-env` without `--yes` is usage exit **2**. Already bound → exit **0**, no rewrite. No path owner → exit **1**, names `register-path`. `--format auto\|human\|json` (same TTY rule as whoami). `context` initializes / rotates; it is **not** adopt-path. Never writes `~/.ai-brains/.env`. Never silent auto-switch (T240 F2). |
...
| **Pin-type re-rank (T211/T215)** | After blend/graph and **before** truncate, hits are re-ranked by a **single composite** (`rerank_hits` — the only post-blend ranking entry point; T215 extends it via ScoreKind, does not add a second final sort). Kind boosts: CONSTRAINT **+4**, DECISION **+2**, HOTSPOT **+0.5**; plan-class DECISION **−3** (optional sibling-track **−2**); shipped DECISION **+1**; T70 stub **−16** composite (`SYMBOL_PENALTY`, when included via `--symbols` or a leak); small recency boost from `updated_at` (including semantic-only hits that SELECT `updated_at`). Sort: effective desc → `updated_at` desc → `memory_id` asc. Content heuristics only — not lifecycle fact (badge `plan/stale?`). |
...
| **Global isolation (T264)** | `--global` stays a vault rollup, not this-repo law. Retrieval prefixes Safety / Index / Recent **item first lines** and Session **headers** with `[` + 8-hex project id + `]` (`[unknown]` when missing). Human/pretty upgrades that leading tag via `display_label` (truncate 32, `]` → `·`). Per-project caps after recency: Safety **2**/vault **8**, Index **3**/vault **15**, Recent **1**/vault **3**, Sessions **1**/project. Summary adds `In context spans N projects` (SQL identity of emitted items; may disagree with marker counts). Summary JSON optional `in_context_project_span` (global only). Project-scoped pretty is unlabeled. Does **not** drop leftover from `recall --global`. |
