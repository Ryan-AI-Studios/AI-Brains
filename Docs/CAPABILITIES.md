...
| Hotspot pin (T321) | `safety sync [--limit N] [--dry-run]` — default **pins** (banner `Pinning N…`); `--dry-run` previews (`would pin`, raw JSON `score`). Parses live `ledgerful hotspots --json` `{schemaVersion, files[]}` (legacy `[…]` stay-green). preflight Safety live-injects without pinning. |
