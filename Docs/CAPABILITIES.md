...
| Feature | Detail |
|---------|--------|
| **FTS5** | Default lexical path; sanitized queries. FTS5 BM25 `rank` is more-negative-is-better (`ScoreKind::Bm25LowerBetter`). |
| **Semantic** | `--semantic` + stored embeddings; honors `AI_BRAINS_EMBEDDING_URL` (default `http://127.0.0.1:8083`) and `AI_BRAINS_EMBEDDING_MODEL` (default `nomic-embed-text-v1.5`). Cosine floor default **0.55** (`AI_BRAINS_SEMANTIC_MIN_SCORE`; one-shot `--min-score`). Candidates below floor are dropped before fusion. Symbols share the same embedding space as decisions — hybrid + floor reduce topic drift but do not eliminate symbol noise. |
| **Hybrid RRF (T215)** | When `--semantic` is on, **FTS ranked list + thresholded semantic ranked list** are fused with pure **Reciprocal Rank Fusion** (`score = Σ 1/(k+rank)`, default **k=60**, env `AI_BRAINS_RRF_K`) **before** bridge merge / graph / `rerank_hits`. Equal weights only. **Missing from a list → no RRF summand** (not rank=`len+1`). Both arms → source `"hybrid"` (FTS content/`updated_at` preferred); FTS-only → `"fts"` (score is RRF contribution under `--semantic`); semantic-only → `"semantic"`. Candidate depth per arm: `max(limit*3, 15).min(50)`; final truncate after pin re-rank. Without `--semantic`: no RRF; blend is bridge → FTS → graph → re-rank. `sync query` stays `semantic: false`. |
...
| **Embedding status** | With `--semantic`, JSON includes additive `embedding: { status, endpoint?, detail? }`. Closed statuses: `ok` \| `unreachable` \| `error` \| `no_stored_embeddings` \| `skipped`. Soft-fail: embed down never aborts FTS/bridge recall (exit **0**). Pretty: status line when `status != ok`; when `status == ok` but **zero** semantic hits above the cosine floor and lexical results are non-empty → `Embedding: ok (no semantic hits above threshold; showing lexical)`. All-below-floor is not an error. |
