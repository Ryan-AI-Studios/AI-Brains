...
| **Index empty-rescue (T346 / T362)** | When **project-scoped** FTS + T105 LIKE retain **0**, the query is Index-shaped, and Index-authority pins exist (same GLOB as preflight Index pass-1, retain `DECISION:` / `CONSTRAINT:` / `INVARIANT:`, not Other, not Hotspot), fill up to `--limit` as `source: "index"` (JSON omits `score`, `score_kind` stays `"bm25"`). Index-shaped means a contentful token in the closed set `decide` / `decision(s)` / `constraint(s)` / `invariant(s)` (T315 `what did we decide` is eligible via `decide`). Porter stems (`chose`, `decided`, `constrained`) do **not** fill. Pretty prints exact `No FTS hits; showing in-scope pins` after Scope/Session/Embedding and before hits; JSON sets `hint` to that SOOT and omit-if-none `fill_kind: "index"`. **Does not** require `--semantic`. **Does not** run on unmatched / contentless queries, `--global`, or when no authority pins exist (T361 `query_miss` / T111 hint stays). Fill is **not** an FTS arm (`has_fts_arm` false). A Bridge hit does not starve fill (Phase 2c before blend). `search` is `recall`. `sync query` and daemon `query_memories` inherit the same Phase 2c gate. `LIST_RECALL_QUERY` unchanged. |
