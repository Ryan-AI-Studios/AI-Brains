...
| **Memory Index two-pass (T274/T286/T327)** | Index pass-1 is leading-marker GLOB **or** TAGS envelope (`index_pass1_glob_sql`: `DECISION:`/`CONSTRAINT:`/`INVARIANT:`/`HOTSPOT:` **or** `TAGS:` / `ASSISTANT: TAGS:`) plus `classify_pin_kind != Other`. A whale pin **does not** abort the rest of pass-1 (no full-body `break`). Non-global Index stops at **15** collected rows (copy-not-share pretty Index cap; `--global` still fetches 80 then round-robin). Pass-2 recency-fills remaining slots only. When pass-1 yield is **0**, Index emits `No DECISION/CONSTRAINT pins in scope; showing recent activity` once (pretty **prologue**, before `1.`). Numbered titles use `first_contentful_line` (skip one role prefix + a `TAGS:` line); empty / role-only / TAGS-only → `Untitled Memory`. Untagged `DECISION:` still wins (T274). Summary `In context decisions` stays a window substring of `DECISION:` (no new JSON key). |
| **Session Other cap (T327)** | Among the **latest 5** loaded turns (pre-existing `turn_projection` LIMIT; not retuned), preview keeps USER turns and authority-prefixed assistant turns; raw Other assistant working turns cap at **3**, then `+K more session turns via recall` (`K` = skipped Other in that window). Pretty recognizes that notice as its own line (standard and `--compact`) and does not synthesize `+N more turns in session` on top. Capture still stores every turn. |
| **Most Recent prefer-authority (T327 / T329)** | Same two-pass as Index (`max_count` 3 non-global). Empty-authority vaults recency-fill. F4 honesty is **Index-only**. Assembly packs Recent **after** Index against leftover budget: skip an oversized body (`continue`), keep smaller collected items; omit the Recent header when nothing packs. Drain still meters by `max_count` only (not full-body size). |
