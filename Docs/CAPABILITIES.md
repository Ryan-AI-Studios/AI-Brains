...
| **Codex CLI** | `harness install --harness codex` → UPS+Stop wrapper → `codex-hook` | **Implemented with caveats (T253):** user-global `~/.codex/hooks.json` only (never rewrite `config.toml`). Feature key is **`hooks`** (not `codex_hooks`). Live SOOT is `prompt` / `last_assistant_message`. **`/hooks` trust required** for live fire — `wiring=ok` means files exist, not that this session trusted the hash. No SessionStart injection. Nightly fifth source (T334). Batch `codex-import` walks `~/.codex/sessions/**/rollout-*.jsonl` (fail-open; format not vendor-stable). Unbound `codex-unbound`. |
