...
| `graph neighbors` / `hierarchy` / `session` | pretty | json | **A.** T246: TTY table; pipe / `--format json` compact JSON. Keys frozen. |
...
| `retention plan` | human | json | **A.** T248: `--format auto` (default). TTY human class matrix; pipe / `--format json` pretty JSON. Keys frozen. Live vaults overlay `memory_legacy` as inventory (`none_auto`): pinned→`held`, other statuses→`skip`; `Nothing to dispose.` means no CE/projection work (T270). |
...
- **Nightly vs Router (T269):** human prints `Nightly: AI-Brains-Nightly` before the schedule block so Last Result **0** is not the same object as `Router: … 267009` (`SCHED_S_TASK_RUNNING`, success). Human `probe=timeout` is labeled `probe=timeout (750ms)` (HTTP `/health` within 750 ms). `--quick` stays `probe=skipped`. JSON probe tokens and the 750 ms budget are unchanged. `daemon status` Open remains TCP connect, not `/health`
