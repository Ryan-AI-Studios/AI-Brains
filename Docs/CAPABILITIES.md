...
| **Role strip display-only (T219)** | Pretty path strips leading case-sensitive `USER:` / `ASSISTANT:` / `SYSTEM:` on index/session/display lines (shared `strip_role_prefix`; same SOOT as `memory list` preview). Stored vault content and non-summary JSON `text` may still embed role labels where assembly emits them. |
