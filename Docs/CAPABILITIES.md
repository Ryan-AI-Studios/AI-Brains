...
| **Index empty-rescue (T346)** | When **project-scoped** FTS + T105 LIKE retain **0** and Index-authority pins exist (same GLOB as preflight Index pass-1, retain `DECISION:` / `CONSTRAINT:` / `INVARIANT:`, not Other, not Hotspot), fill up to `--limit` as `source: "index"` (JSON omits `score`, `score_kind` stays `"bm25"`). Pretty prints exact `No FTS hits; showing in-scope pins` after Scope/Session/Embedding and before hits. **Does not** require `--semantic`. **Does not** run on contentless queries, `--global`, or when no authority pins exist (T111 hint stays). Fill is **not** an FTS arm (`has_fts_arm` false). A Bridge hit does not starve fill (Phase 2c before blend). `search` is `recall`. `LIST_RECALL_QUERY` unchanged. |
