...
| **Summary session card (T345)** | After `Scope:`, project-scoped human prints `path=<compare>` or `path=—` (other-owner is unbound). Leftover T282 line when shell PROJECT_ID ≠ cwd `.env`. `last_decision:` is the first budget-window `DECISION:` remainder (marker stripped, 100 UTF-8 bytes). **Exactly one** `next:` line: unowned location → `ai-brains context`; else incomplete discovery grants → bootstrap SOOT; else T315 empty-decisions. `--show` is unchanged. Query-only (no bind). |
