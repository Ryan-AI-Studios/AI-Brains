...
| `graph neighbors` / `hierarchy` / `session` | pretty | json | **A.** T246: TTY table; pipe / `--format json` compact JSON. Keys frozen. T278: session PREVIEW is `{n} memories · first line` (human-only). |
