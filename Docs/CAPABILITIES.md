...
| `graph rebuild` | Full resync (recovery). Stop the daemon first (`ai-brains daemon stop` / `sc stop AI-Brains-Daemon`); mutating rebuild fail-closes while the daemon is up (exit **1**). Prefer `--dry-run` (prints current density + would-DELETE sentence; allowed while daemon is up). Success stdout is the density report (human default; `--format json` same keys as `graph update`). May still report `sparse` after replay (typed-lineage floor **0.50** stays honest). T246 does **not** rebuild automatically. |
