...
| `briefing` | markdown | markdown | **B.** T349: omitted `--format` stays markdown even when piped. `human\|pretty\|text\|markdown\|md` → markdown; only `json` → JSON; unknown → exit **2**. `--format json` for scripts. |
...
| `scope resolve` | human | human | **B.** T349: omitted `--format` stays human even piped. `--format auto` still TTY human / pipe JSON. `--format json` for scripts. Keys frozen. Tokens case-sensitive (`JSON`/`Pretty` exit 2). |
| `project list-paths` | human | human | **B.** T349/T266/T254: omitted `--format` stays table even piped. Tokens `auto\|pretty\|human\|text\|json\|markdown\|md`. `--format pretty` ≡ table. `--format auto` still pipe JSON. Scripts: `--format json`. Keys frozen. |
...
| `project whoami` | human | json | **A.** Same token map. Identity remediator, not a default flip. **T349:** path-null slug-miss env fallback prints detect’s `git/env project mismatch` on **human stderr**; JSON stderr silent of that SOOT; no JSON `warning` key; `mismatch` stays env vs path-alias. |
...
| `graph neighbors` / `hierarchy` / `session` | pretty | pretty | **B.** T349: omitted `--format` stays table even piped. `--format auto` still pipe JSON. `--format json` compact. Keys frozen. T278: session PREVIEW is `{n} memories · first line` (human-only). T293: human prefer-fills authority 1-hop (`DECISION:`/`CONSTRAINT:`/`INVARIANT:`/`HOTSPOT:`); JSON array order stays direction→label→id. **T317:** human neighbors caps `RECALLS` at **3** + `+N more RECALLS` (header stays full 1-hop N); JSON lists all. Hierarchy leaf pretty may add `next: ai-brains nightly --status` (still no graph update/rebuild). |
| `graph update` | json | json | **C.** T246 F6 / T74: default pretty JSON. `--format auto` stays JSON (no TTY switch). `--format human` opt-in labeled lines. **T326:** pinned COUNT fail is command error (exit **1**), not fake `pinned_memories: 0`. |
...
| `project list` | human | human | **B.** `--format human` (default) or `json`. No `auto`. **T349:** human hides non-cwd 0-memory rows and caps at 20 (`+N more (ai-brains project list --all)`); cwd-owner stays first even at 0 memories. JSON / `--all` keep every row. |
...
| `status` | human | human | **B.** T349: omitted `--format` stays human even piped. `--format auto` still TTY human / pipe json. Tokens `auto\|pretty\|human\|text\|json\|markdown\|md` (case-sensitive). T320 unified glance: daemon IPC Running/Stopped + doctor attention + graph density one-liner + nightly last-run/scheduled. Fail-open per section; exit **0** always for a successful emit (degraded / Stopped / sparse / never). **T326:** pinned-memory COUNT fail is graph section `error` (human `graph: error=…`; JSON `graph.error` nonempty, other graph keys omitted) — not `pinned=0` then `live`/`skip`. Does **not** replace `doctor` / `nightly --status` / `daemon status` / `graph update`. No HTTP probes; no daemon TCP. CLI-local envelope `schema_version: 1` (not a contracts DTO). Stopped may add `next: ai-brains daemon start` / JSON `next_step`. |
| `daemon status` | human | human | **B.** No `--format`. Unset `AI_BRAINS_MODEL_URL` / `AI_BRAINS_EMBEDDING_URL` probe nightly **8081 / 8083** (T349; not Ollama `:11434` / llama.cpp `:8080`). When Stopped and LLM/Embedding TCP is Open, prints `backend TCP Open ≠ daemon` (IPC vs model TCP — not nightly’s T281 `HTTP /health 750ms ≠ daemon TCP`) **before** the last line. Stopped last line: `next: ai-brains daemon start`. Running omits contrast and `next:`. Exit **0** both states. Keyless liveness (T199). |
...
| `retention plan` | human | human | **B.** T349: omitted `--format` stays human even piped. `--format auto` still TTY matrix / pipe JSON. `--format json` pretty JSON. Report keys frozen. Live vaults overlay `memory_legacy` as inventory (`none_auto`): pinned→`held`, other statuses→`skip`; `Nothing to dispose.` means no CE/projection work (T270). Human **Work** lists dispose identities (class CE/projection counts), not the class dominant mechanism — a held-majority `secret` still shows a `ce_wipe` Work row (T284). |
| `decision in-force` | human | human | **B.** T349: omitted `--format` stays human even piped (`POLICY_DENIED:` + SHORT). `--format json` keeps `code` + `details.hint` = LONG `POLICY_DENIED_HINT`. Optional positional `TERM` or named `--term` (`num_args 0..=1`, `default_missing_value ""`; use `--term=` for empty — PowerShell 5.1 drops `""`). Omit / empty → `fail_usage` exit **2** (`term must be non-empty`), not clap missing `<TERM>`. Optional `--as-of <RFC3339>` hop-stops the supersede chain (omit = now). `--format` `value_parser` tokens `auto\|pretty\|human\|text\|json\|markdown\|md`; unknown → clap exit **2**. `auto` is not a TTY switch. Human/pretty/text: optional `As of:` + `Term:`/`Scope:`/`Ruling:` or F12 none + recall next. Local projection; `ReadDecisions`. |
| `conclusion in-force` | json | json | **C.** T323/T324: default JSON. Same optional `TERM` / `--term` / `--term=` empty path as decision (T324). Walks `superseded_by` to the current Active\|Confirmed tip (no `--as-of`). `--format` `value_parser` tokens `auto\|pretty\|human\|text\|json\|markdown\|md`; unknown → clap exit **2**. `auto` is not a TTY switch. Human/pretty/text: `Term:`/`Scope:`/`Ruling:` (statement) or F12 none + recall next. Local projection; `ReadConclusions`. |
...
| **Capture coverage (T337/T348/T360)** | `capture coverage` | Read-only disk vs vault `SessionStarted`. `wiring=ok` is presence, not capture proof. Numbered disk `>` vault → `deficit` (importer next step). Grok `eligible > vault` → `unverifiable_subagent` + `grok-import --days N --dry-run` (never `--force`). Both empty → `never_exercised`. Exit **0** on those statuses. Not a 16th doctor check. |
| **agy (Antigravity CLI)** | `harness install --harness agy` → Stop wrapper → `agy-hook --payload` | **Implemented (T235+T236+T245+T358):** always merge official IDE `~/.gemini/config/hooks.json` managed key `ai-brains-capture`. **Iff** `~/.gemini/antigravity-cli` already exists, also stage CLI plugin bundle `plugins/ai-brains-capture/{plugin.json,hooks.json}`. Never create `antigravity-cli` just for plugins. Never write undocumented top-level `antigravity-cli/hooks.json`. Stop `command` is shell-form with first token `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe` (quoted only if `SystemRoot` has whitespace). Wrapper stdout = allow-stop JSON only; step-shaped + full transcript prefer; path normalize; env fallback only for `agy-unbound`. Reinstall after T236 / after CLI upgrade (baked path). |
...
| **Grok Build** | `harness install --harness grok` → Stop/SessionEnd wrapper → `grok-hook` | **Implemented with caveats (T237+T357+T358):** `~/.grok/hooks/ai-brains.json` + `~/.ai-brains/hooks/grok-capture.ps1`. Native Stop/SessionEnd `command` is shell-form whose first token is `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe` (quoted only if `SystemRoot` has whitespace; no `$`). **Stop allow = empty stdout** (never AGY `{"decision":"allow"}`). User keep: non-empty `<user_query>`/`<USER_REQUEST>` only (chrome/`synthetic_reason` dropped). Subagent/worktree sessions skipped by default. `source_ts` usually none → `occurred_at` = ingest time. Turn ids `v5(session,"turn-{i}")` on kept index (filter taxonomy change can shift ids). Vendor-compat: `harness install --harness grok` / `all-ready` merges `compat.claude.hooks = false` into `~/.grok/config.toml` (format-preserving; skip on parse/non-table/write failure). That cell is **global** — it stops home and project `.claude/settings.json` scans. `GROK_CLAUDE_HOOKS_ENABLED` still wins over toml. Cursor compat is unchanged. Native capture stays `~/.grok/hooks/ai-brains.json` → `grok-capture.ps1`. |
...
| **Claude bulk** | `claude-import --days N [--force] [--dry-run]` | Project JSONL; `--force` skips 300s quiescence; nightly fourth source (T334) |
| **Codex CLI** | `harness install --harness codex` → UPS+Stop wrapper → `codex-hook` | **Implemented with caveats (T253+T358):** user-global `~/.codex/hooks.json` only (never rewrite `config.toml`). Feature key is **`hooks`** (not `codex_hooks`). UPS/Stop `command` is a single shell-form string whose first token is `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe` (no `args`, no `commandWindows`). Live SOOT is `prompt` / `last_assistant_message`. **`/hooks` trust required** for live fire — `wiring=ok` means files exist, not that this session trusted the hash. No SessionStart injection. Nightly fifth source (T334). Batch `codex-import` walks `~/.codex/sessions/**/rollout-*.jsonl` (fail-open; format not vendor-stable). Unbound `codex-unbound`. |
| **codex-hook** | `codex-hook --payload '{...}'` | Live message-only ingest; `--schema` (vault-path-free). Missing fields exit 0. |
| **Codex bulk** | `codex-import --days N [--force] [--dry-run]` | Rollout JSONL keep `response_item` message roles only; nightly fifth source (T334) |
| **Cursor IDE** | Batch only (`cursor-import`) | **Partial (T334):** no live hooks, no `harness install --harness cursor`, no doctor sixth row. Walks `~/.cursor/projects/<slug>/agent-transcripts` (nested uuid folders + flat jsonl). Message-only after `filter_turn`; strips `<manually_attached_skills>` / `<timestamp>`; skips `subagents/`; does not open Composer `state.vscdb`. Bind is case-insensitive slug of `list_path_aliases`. Unbound `cursor-unbound`. |
| **Cursor bulk** | `cursor-import --days N [--force] [--dry-run]` | Agent-transcripts JSONL; `--force` skips 300s quiescence; nightly sixth source (T334) |
...
| List projects | `project list` — **label-first** human table: `label` \| `project_id` (full) \| `memories` \| `last_activity` \| `path`. Label prefers alias; baked/machine names show as `(no alias)`. Active process `AI_BRAINS_PROJECT_ID` gets a `*` prefix on the label. Human table puts the cwd path-owner first even at 0 memories; hides other 0-memory rows; caps at 20 with `+N more (ai-brains project list --all)`. JSON / `--all` keep every row (JSON array order stays T212 size-desc). |
...
| List all path aliases (T254/T259/T266/T349/T355) | `project list-paths` — every registered root (not just `project list` first-path). Optional `--project <id\|alias>` and `--shared-only` (owner appears on ≥2 roots). Combined flags are an intersection. Omitted `--format` stays **human even when piped**; `pretty\|human\|text\|markdown\|md` → table; `json` → object; `--format auto` still TTY table / pipe JSON. Tokens case-sensitive (`JSON`/`Pretty` exit **2**). JSON `{ api_version:"1", paths:[{project_id,label,alias,normalized_path,exists}] }` ASC by path (raw projection rows; Win+WSL register of the same location is **one** row). Human table additionally collapses any location-equal strings for the same project (first ASC row wins). `--shared-only` JSON cardinality is raw-row (`owner_counts`), not human-collapsed. Unfiltered empty: `No path aliases registered.` + `next: ai-brains project register-path <project_id\|alias> <path>` (exit **0**). Filtered empty: `No path aliases match.` / `paths: []` (exit **0**). Unknown `--project` dest exit **1**. |
...
| Auto-detect (T240/T333/T361) | `project detect` order: **(1)** path alias of git toplevel else cwd → **(2)** git slug exact-first (T206; ambiguous exit **1** when no path) → **(3)** env `PROJECT_ID` post-dotenv if in vault → **(4)** miss exit **1**. Path owner **always** wins over unique slug hit; stderr notes the slug project (extra note when path has 0 memories and slug has &gt;0). Human and `--export` comments lead with `detect_lead_label` (human alias wins; path-like alias keeps a human stored `name`; baked `(no alias) — uuid` is never the lead when a human alias exists). `--export` comments include `source=path_alias` / `git_slug` / `env`. **`--format json`** (T266 tokens; omitted `--format` stays **human even when piped**; `--export` XOR `--format`) emits always-present `source` (`path_alias` \| `git_slug` \| `env` \| `none`; never omit / never `null`). Success: `project_id`, `name`, `alias`, additive `label` (`detect_lead_label`), `memories` (real COUNT), `source`, `notes` (E1 `[]`); `warning` omitted when none. Miss/ambiguous: `source: "none"`, identity fields including **`label` JSON `null`** (keys present; memories not `0`), `message` on miss, `notes` on ambiguous, exit **1**, JSON on stdout. |
| Whoami (T240/T266/T328/T332/T349) | `project whoami` — all identity signals: `effective_project_id`, `env_project_id` (post-dotenv; null under `--no-project-context`), `shell_project_id` (pre-dotenv when set/differs), `path_alias_project_id`, `detect_project_id`, **`detect_source`** (`path_alias` \| `git_slug` \| `env` \| `none`; always a string; never omit / never `null`; `none` when detect id is absent or slug is ambiguous), `git_slug`, `git_toplevel`, `mismatch`, **`identity_collision`** (always a boolean; never omit / never `null`), `remediations[]`. `--format auto` (TTY human / pipe JSON); same T266 token map as list-paths. Does **not** rewrite `PROJECT_ID`. `mismatch` stays env vs path-alias (both Some). **T328:** when env ≠ detect and path-alias is absent, `identity_collision: true` and remediations name `register-path` first (then `adopt-path`); collision is honesty, exit **0**. **T332:** when detect falls back to env after a git-slug miss (env==detect, path-null), `identity_collision` stays **false** and remediations name `set-alias` / `register-path` (not a git hit). **T349:** that slug-miss path also prints detect’s `git/env project mismatch` on **human stderr**; `--format json` stderr stays empty of that SOOT (no JSON `warning` key). On mismatch, remediations name `project adopt-path` (print-only / `--write-env --yes`) plus the exact `AI_BRAINS_PROJECT_ID=<path-id>` assignment — not `project list`, not “run whoami”. |
...
| **Pretty scores (T218 F6 / T346 F4)** | Branch on ScoreKind: **RRF / HigherIsBetter** → primary **`rank=#n`** + **`sim=0.XX`** when cosine known (do **not** primary-print raw RRF ~0.016). **BM25** hides `score=` on pretty (memory id stays). **Bridge** keeps readable raw `score={:.3}`. JSON still emits numeric BM25 `score` + `score_kind=bm25`. |
...
| **Contentless query (T261)** | Zero contentful tokens (`""`, whitespace, punctuation-only, all-stopwords, single-char-only such as `"a"` / `"i"`) is the existing T207 empty envelope: no FTS rescue, no substring LIKE, no Index fill (T346), no Ledgerful bridge, no embed HTTP, no graph expansion, no `MemoryPinned`. `--symbols` does **not** override. `--semantic` + contentless sets `embedding.status=skipped`, `detail=contentless_query`, `endpoint=None` (no provider call). Apostrophe fragments stay T217 per-token (`"can't"` contentless; `"i'll"` / `"don't"` contentful). `forget --match` stays unfiltered (all-stopword still MATCH). Piped `recall -` with empty/whitespace stdin is this empty envelope (exit **0**); TTY `recall -` still refuses. |
| **Pin-type re-rank (T211/T215/T274/T285/T312/T325)** | After blend/graph and **before** truncate, hits are re-ranked by a **single composite** (`rerank_hits` / `rerank_hits_with_query` — the only post-blend ranking entry point; T215 extends it via ScoreKind, does not add a second final sort). Kind boosts: CONSTRAINT **+4**, DECISION **+2**, HOTSPOT **+0.5**; plan-class DECISION **−3** (optional sibling-track **−2**); shipped DECISION **+1**; T70 stub **−16** composite (`SYMBOL_PENALTY`, when included via `--symbols` or a leak); session-chrome **−16** (`SESSION_CHROME_PENALTY`) for closed-list dumps (`## Objective`, `# Track Plan Review`, `# AI-Brains Session Onboarding`, `# Review of Track`, …) **and** ATX headings whose FTS tokens include `review` / `objective` / `onboarding` / `audit` / `ratings` (token set, not substring — `# Preview of graph` is not chrome). Verbose-Other dumps (≥800 Unicode chars, non-chrome Other) also take **−16** (`DUMP_OTHER_PENALTY`, never stacked with chrome). Kind markers are **leading-line only after pin envelope** (strip one `USER:`/`ASSISTANT:`/`SYSTEM:`, skip a `TAGS:` line; buried `decision:` / JSON keys are Other; leading `INVARIANT:` is Constraint). Authority first-line query-token overlap **+16** (`LEADING_QUERY_BONUS`) inside the same sort. Lexical MATCH is **two-pass**: authority GLOB **or** `TAGS:` envelope first, in-memory retain, recency retry when retain is empty; **T312/T325 authority-OR fill** when retain is still empty and the query has ≥2 contentful tokens (Prefer `match_or`, then PreferRecency on the OR expr when Prefer-OR retain is empty — same empty-only retry as AND); pass-2 uses the OR expr once OR-fill or its recency retry retains; AND pass-2 only when both fills are empty. **T364:** Index-shaped queries (`index_fill_eligible`) with `exclude_symbol_stubs` skip that pass-2 remainder (`--symbols` keeps it). Session dumps stay recallable; they do not occupy the whole `candidate_depth`. Near-duplicate chrome sharing a first line collapses after rerank. Sort: effective desc → `updated_at` desc → `memory_id` asc. Content heuristics only — not lifecycle fact (badge `plan/stale?`). No new Recall JSON keys. KIND / floors / `candidate_depth` frozen. |
| **Graph boost** | Neighbor score boost (`--graph-boost`); expansion is **hop-1 by default** (including lexical, not only `--semantic`). Chrome-shaped and verbose-Other parents do not seed neighbors (T260 stubs + T285 session chrome + T312 F10). |
| **Substring fallback** | When FTS empty on small vaults (**contentful** misses only). Contentless queries never COUNT/LIKE (T261). |
| **Index empty-rescue (T346 / T362 / T364)** | When **project-scoped** FTS + T105 LIKE retain **0**, the query is Index-shaped, and Index-authority pins exist (same GLOB as preflight Index pass-1, retain `DECISION:` / `CONSTRAINT:` / `INVARIANT:`, not Other, not Hotspot), fill up to `--limit` as `source: "index"` (JSON omits `score`, `score_kind` stays `"bm25"`). Index-shaped means a contentful token in the closed set `decide` / `decision(s)` / `constraint(s)` / `invariant(s)` (T315 `what did we decide` is eligible via `decide`). Porter stems (`chose`, `decided`, `constrained`) do **not** fill. **T364:** default recall (`exclude_symbol_stubs`) skips prefer-authority pass-2 chrome remainder and skips T105 LIKE on Index-shaped queries so quoted-command reviews cannot occupy `results` and starve fill. `--symbols` still runs pass-2. Pretty prints exact `No FTS hits; showing in-scope pins` after Scope/Session/Embedding and before hits; JSON sets `hint` to that SOOT and omit-if-none `fill_kind: "index"`. **Does not** require `--semantic`. **Does not** run on unmatched / contentless queries, `--global`, or when no authority pins exist (T361 `query_miss` / T111 hint stays). Fill is **not** an FTS arm (`has_fts_arm` false). A Bridge hit does not starve fill (Phase 2c before blend). `search` is `recall`. `sync query` and daemon `query_memories` inherit the same Phase 2c gate. `LIST_RECALL_QUERY` unchanged. |
...
| Feature | Detail |
|---------|--------|
| **Briefing format (T227/T349)** | Omitted `--format` stays **markdown even when piped**. Explicit aliases `human`, `pretty`, `text`, `markdown`, `md` → **markdown**; only `json` → JSON. Unknown `--format` → **exit 2** (`fail_usage`) + accepted list on stderr, **zero stdout**. Scripts pass `--format json`. |
...
| **Denied packets** | Project briefing: `denied=true` seeds `warnings[]` `kind: "denied"` + bootstrap next-step (`policy bootstrap`); exit **0**. **T275/T349:** Denied **human** is Denied + SHORT (+ optional grant-wall one-liner); **no** empty `## Decisions` / hidden dump. JSON keeps `denied` / `decisions` / `conclusions` keys. Pins remain via `recall` / `search`. After CLI `policy bootstrap` (System default — omit `--principal-id`), `briefing project` is `denied: false` and `evidence list` exits **0**. Personal briefing deny names **`recall`** (optional continuity; not a required bootstrap); exit **0**. **T289:** Denied Personal human omits `_None_` under Preferences/Continuity and uses `_(optional continuity; not a missing vault)_` — not empty prefs / not a required Personal bootstrap. JSON stays `denied: true` + empty arrays (no new keys). Allowed-empty Personal still `_None_`. |
...
| **Show** | `source show` / `evidence show` use the same soft-resolve helper. **T319:** a vault `memory_id` pasted into show/expand is **named** (not shown as evidence/source) and points at `ai-brains recall "what did we decide"`; expand/evidence Unknown stay exit **0**; source miss stays exit **4** + additive hint |
...
| **`--compact` (T250)** | Tighter display caps: safety **3** / turns **2** / sessions **1** / index **5** / recent **2**. First-line-only on Safety/Recent; line-cap **100** on Session + Recent + Safety first line. F31 `+N` notices still fire with compact N. Compact Recent keeps the `(Use 'recall'…)` hint. **T327:** compact caps still apply **after** retrieval selection (authority-first Index/Session/Recent); retrieval does not retune compact constants. |
...
| **Ledgerful hotspots (T279 / T321 / T347)** | Pretty **Repository Bearings & Safety** is live `ledgerful hotspots --json --limit 5` (`HOTSPOT: <path> score=n`, project-scoped; parses `{schemaVersion, files[]}` or legacy `[…]`) plus leading-line GLOB `CONSTRAINT:` / `INVARIANT:` / `HOTSPOT:` (not LIKE-anywhere; dumps with buried markers do not steal the section). After parse, skip path components `deps_src` / `third_party` / `vendor` / `.git` (case-insensitive; `\` ok) and skip `score <= 0` (NaN/-inf). `--global` does **not** live-inject (cwd scan is one repo). Honest empty still emits the Safety header and `No repo-local hotspots above threshold. next: ai-brains safety sync --dry-run`. Default `safety sync` **pins** remaining rows (banner + about/after_help); `--dry-run` previews; `--include-zero` keeps zero scores (path deny still applies); preflight does not pin and has no include-zero flag. Bridge Intelligence (`bridge export --hotspots`) is a separate section and stays off under `--global`. |
| **Memory Index two-pass (T274/T286/T327/T330)** | Index pass-1 is leading-marker GLOB **or** TAGS envelope (`index_pass1_glob_sql`: `DECISION:`/`CONSTRAINT:`/`INVARIANT:`/`HOTSPOT:` **or** `TAGS:` / `ASSISTANT: TAGS:`) plus `classify_pin_kind != Other`. A short `DECISION:` / `CONSTRAINT:` pin is **not** skipped as low-signal (`word_count < 6` applies to Other only). A whale pin **does not** abort the rest of pass-1 (no full-body `break`). Non-global Index stops at **15** collected rows (copy-not-share pretty Index cap; `--global` still fetches 80 then round-robin). Pass-2 recency-fills remaining slots and **skips session chrome** (`## Objective` / onboarding / review dumps) — chrome does not occupy empty-authority Index slots. F4 `No DECISION/CONSTRAINT pins in scope; showing recent activity` fires when **no collected Index item** is authority (not merely GLOB pass-1 length 0), including chrome-only project-scoped vaults with zero numbered items. Empty-repo (`collected` empty, no active sessions, not `--global`, no project id) emits the new-repo banner only — no Index header / F4 / Recent. Numbered titles use `first_contentful_line` (skip one role prefix + a `TAGS:` line); empty / role-only / TAGS-only → `Untitled Memory`. Untagged `DECISION:` still wins (T274). Summary `In context decisions` stays a window substring of `DECISION:` (no new JSON key). |
| **Session Other cap (T327 / T330)** | Among the **latest 20** loaded turns (`turn_projection` `SESSION_TURN_FETCH`), preview keeps USER turns and authority-prefixed assistant turns; raw Other assistant working turns cap at **3**, then `+K more session turns via recall` (`K` = skipped Other in that window, not a whole-session COUNT). Pretty recognizes that notice as its own line (standard and `--compact`) and does not synthesize `+N more turns in session` on top. Capture still stores every turn. |
| **Most Recent prefer-authority (T327 / T329 / T330)** | Same two-pass as Index (`max_count` 3 non-global) **without** Index pass-2 chrome skip (dumps-only vaults still recency-fill Recent). F4 honesty is **Index-only**. Assembly packs Recent **after** Index against leftover budget: skip an oversized body (`continue`), keep smaller collected items; when nothing packs and leftover can hold header+footer+≥8 body words of the first collected item, emit a truncated snippet (`trim_to_word_budget_no_sentinel`). Drain still meters by `max_count` only (not full-body size). |
...
| **Ledger pane (T271 / T313)** | Ledgerful `ledger search` is a **phrase** search (it wraps the whole argv). The CLI forwards `strip_ansi(query).trim()` — **not** T90 FTS AND quotes. The spawn always inserts POSIX `--` immediately before QUERY so dash-leading needles (`--limit`, `--days`) are search text, not Ledgerful flags (T273; operator form: `sync query -- --limit`). Multi-token phrase misses retry up to 3 first-seen contentful tokens; first hit wins with heading `--- Ledgerful Ledger Search (rescued token: '<tok>') ---` plus `Note: no phrase match for '<user>'; showing hits for '<token>'.` Phrase hits / misses keep the generic `--- Ledgerful Ledger Search ---` heading. Named misses: never-ran / failed / ran-empty (user query, never `'"tok" "tok"'`). `--no-bridge` skips the pane. `--quiet` omits never-ran/failed (not ran-empty/hits). |
...
6. Embedding backfill + stale refresh + WAL checkpoint (UTF-8-safe per-chunk truncate — T229 F5 / T351 chunk+mean-pool)
...
| Command | Purpose |
|---------|---------|
| `graph update` | Health report: default remains **pretty JSON** (T213 keys: `nodes`, `edges`, `pinned_memories`, `memory_nodes`, `edge_node_ratio`, `density` (`ok`\|`warn`\|`skip`), `status` (`live`\|`sparse`\|`empty`), `note`, optional `remediation`). Opt-in `--format human` prints labeled lines. `--format auto` does **not** TTY-switch (stays JSON). Feature-off still exit **2**. **T326:** pinned-memory COUNT fail is command `Err` (exit **1** `COMMAND_FAILED`, analog tables-missing) — not fake JSON `pinned_memories: 0` / `status: live`. |
| `graph rebuild` | Full resync (recovery). Stop the daemon first (`ai-brains daemon stop` / `sc stop AI-Brains-Daemon`); mutating rebuild fail-closes while the daemon is up (exit **1**). Prefer `--dry-run` (prints current density + would-DELETE sentence; allowed while daemon is up). Success stdout is the density report (human default; `--format json` same keys as `graph update`). May still report `sparse` after replay (typed-lineage floor **0.50** stays honest). T246 does **not** rebuild automatically. **T326:** pinned COUNT fail uses the same shared health builder as `graph update` — command error, not fake `pinned_memories: 0`. **T335:** session-less symbol pins (`MemoryPinned` with `session_id` none + `project_id`) get `PINNED_IN_PROJECT` (memory → project) on live ingest and on rebuild; no `graph repair` command. CLI `pin` already requires `AI_BRAINS_SESSION_ID`. |
| `graph neighbors <memory_id>` | 1-hop neighbors. TTY pretty human (or `--format pretty`/`human`/`text`); **compact** JSON when piped or `--format json`. `--limit`/`-l` (pretty default 50 max 200; JSON unlimited unless `--limit` given). Pretty columns DIR/LABEL/ID/KIND/PREVIEW: `memory` is the content line; `session` is `{n} memories · first line` (T278); other kinds stay blank. T293: human prefer-fills authority 1-hop (memory/session leading `DECISION:`/`CONSTRAINT:`/`INVARIANT:`/`HOTSPOT:`) before dump sessions; JSON order unchanged (direction→label→id). **T317:** after prefer, human caps `RECALLS` at **3** and prints `+N more RECALLS`; header `(N)` is the full pre-cap 1-hop count; JSON still lists every 1-hop. The id printed by `pin` is this memory id (and the graph node) after T262. Empty pretty: no graph node + vault has the memory → `next: ai-brains graph rebuild`; unknown id → `(not a vault memory id)` with **no** remediator; present-but-empty edges have **no** remediator. Never `graph update` for a missing node (`update` is health-only). JSON keys unchanged. |
| `graph hierarchy <memory_id>` | Synthesis chain. Same TTY/pipe `--format` / `--limit` as neighbors. Empty pretty distinguishes no graph node (same missing-node next as neighbors) vs honest leaf (`No SYNTHESIZED_FROM children (leaf).` + **T317** `next: ai-brains nightly --status`; still **no** graph update/rebuild remediator). JSON keys unchanged (`synthesized_from: []` on leaf). |
...
| Hotspot pin (T321 / T347) | `safety sync [--limit N] [--dry-run] [--include-zero]` — default **pins** remaining rows (banner `Pinning N…`); `--dry-run` previews (`would pin`, raw JSON `score`). Skips vendored path components and `score <= 0` unless `--include-zero`. Filter-empty prints `SAFETY_EMPTY` (no pin). Parses live `ledgerful hotspots --json` `{schemaVersion, files[]}` (legacy `[…]` stay-green). preflight Safety live-injects without pinning. |
...
Default/Quiet `backup list` table = **usable encrypted backups only** (`Readable \| PreT109` via `is_usable_class`); residual classes never appear as Default/Quiet table rows. When any residual exists, Default prints one **stdout** footer counting legacy plain / incomplete / key / corrupt (`not recoverable under current key` … `--verbose` … `verify`); Quiet omits the footer. `--verbose` lists every class (usable-first) plus per-file WARN detail and omits the footer. Residuals-only fleet → `No usable backups.` (empty dir still `No backups found.`). CLI sort stays T244 usable-first (then newest timestamp; unparseable last within band); brain `list_backups` stays timestamp-desc for doctor. Dual `--quiet --verbose` → quiet wins. Exit **0** for any mix of classes after a successful scan.
...
**Backup verify quiet default (T225 / T244 / T318):** human `backup verify` prints **counts** (`Verified N backup(s): X OK, Y FAIL.`). Mixed fleet (`ok >= 1` and `fail >= 1`): counts + `{fail} FAIL (use --verbose for per-file).` — **no** first-5 `FAIL —` dump. Zero-OK (`ok == 0 && total >= 1`): T225 first **5** `filename: FAIL — {reason}` lines (T138 reason preserved) + overflow trailer when fail &gt; 5 + create nudge (`ai-brains backup create`) **unchanged**. OK lines omitted. Verify requires **both** core tables (`missing core tables` when fewer than two); JSON `tables` still lists whichever of `events`/`memory_projection` were found. **`--verbose`** = full per-file OK/FAIL stream only (no summary, trailer, or nudge). **JSON** always returns full `results[]` (verbose ignored). Progress tracing is `debug` (no INFO flood under product default `RUST_LOG`). Any FAIL → exit **1**; empty → exit **0** (`No backups to verify.`).
